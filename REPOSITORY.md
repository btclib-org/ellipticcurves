...
No sentinel appears in the rule, and none of them may: `links.yml`,
`deps-latest.yml`, `deps-oldest.yml`, the three `os-*` workflows,
`pypi-install.yml` and `sdist-rebuild.yml` can each go red for reasons no
pull request introduced, and a red check nobody can act on from a branch is noise.
