...
"""The ellipticcurves package: what it publishes, and the version metadata.
...
still an attribute here: `ellipticcurves.__version__` is how a caller
reads the version and `ellipticcurves.name` how it reads the name.
...
ellipticcurves` is the metadata lookup below and nothing else, and the
...
as a promise that any attribute may exist, so `ellipticcurves.cruves` is
...
actually writes -- `from ellipticcurves import curves`, `import
ellipticcurves.curves`, `from ellipticcurves.curves import mult` --
...
name = "ellipticcurves"
try:
    __version__ = version("ellipticcurves")
...
def __getattr__(published: str) -> ModuleType:
    ...
    so it answers `ellipticcurves.curves` once and the import machinery's
    own attribute answers it from then on -- and it never runs for
    `import ellipticcurves.curves` or `from ellipticcurves import curves`,
    which import the submodule themselves. What it makes work is
    `getattr(ellipticcurves, "curves")` on a fresh interpreter, which is
    how a walker reading `__all__` descends, and `from ellipticcurves
    import *`, which asks for each name in the list.

    Anything not in `__all__` raises `AttributeError`, private modules
    included: `import ellipticcurves._utils` still reaches one. The
...
def __dir__() -> list[str]:
    ...
    `dir(ellipticcurves)` consults this rather than the namespace, so
