...
class Sig:
    ...
    def assert_valid(self) -> None:
        ...
        # _is_x_coordinate_var and not the _y_even_var that was here (issue
        # btclib-org/btclib#622). The delegated lift was already cheap on the
        # accepting path, and what it saves there is not the reason: _y_even_var
        # falls back to ec.y_even_var for an x the bindings refuse -- that being
        # where the message naming the value comes from -- so refusing cost the
        # whole Python square root, several times what verifying a good
        # signature costs. The expensive answer was the one an attacker picks,
...
class Signer:
    ...
    **What this hands the caller is the lifetime of a secret.**
    ...
    def sign_(
        ...
        # no size on its message. Signing through it would put back the gate
        # issue btclib-org/btclib#169 removed -- and would put it back in only
        # one of this class's two arms, the python one signing what the
        # delegated one refused
        #
...
def assert_as_valid_(
    ...
    *,
    ...
    # sign_. `libsecp256k1_ssa.verify` has always taken BIP340's message of any
    # size -- what sent the four arbitrary-size vectors of issue
    # btclib-org/btclib#169 down the Python path was the 32-byte gate in front
    # of it, never the call itself, and they are verified here now
