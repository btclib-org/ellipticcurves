...
from ellipticcurves.alias import HashF, Integer, Octets, Point
from ellipticcurves.curves import Curve, bytes_from_point, mult, secp256k1
from ellipticcurves.curves.curve import _add, _assert_valid_ec
...
from ellipticcurves.number_theory import legendre_symbol_var, mod_inv, mod_sqrt_var
...
def second_generator(ec: Curve = secp256k1, hf: HashF = sha256) -> Point:
    ...
    recomputing it on every call cost 71% of a commitment (issue
    btclib-org/btclib#287).
...
def commit(r: Integer, v: Integer, gen: Point, ec: Curve = secp256k1) -> Point:
    ...
    r=0 mod n is refused: it commits with no blinding at all, Q is then v*gen, a
    point anyone who guesses v can recompute. The check is on r alone and not on
    its range, because the sum of two blinding factors is a blinding factor too
    -- a Pedersen commitment is additively homomorphic -- and is routinely >=
    ec.n (issue btclib-org/btclib#1250). It also subsumes the former separate
    check for r and v both landing on INF: with r=0 mod n excluded, Q lands
    there only if v is 0 mod n too, which is an ordinary commitment to a zero
    value and not a blinding failure.

    Checked here and nowhere else in the module: `assert_as_valid`
    recomputes the commitment through this function, and `verify`
    already turns the `EllipticCurvesValueError` this raises into `False`, the
...
def assert_as_valid(
    ...
    The type of the commitment is checked and its *value* is not: a `None`
    compares unequal to every point, so `verify` reported a commitment of no
    type at all as one that does not open, where a pair of ints that is no
    commitment is exactly what False is for (issue btclib-org/btclib#814).
    `is_on_curve` is deliberately not asked -- that would refuse a wrong value
    too.
    """
...
def _assert_structurally_valid_(r: Integer, v: Integer) -> None:
    ...
    made under -- into False (issue btclib-org/btclib#2170).
    ...
    The commitment and the generator are asked nothing here. They enter as
    native points and no parse of them runs at all: `assert_as_valid` checks the
    commitment's type and deliberately not its value, a pair of ints that is no
    commitment being exactly what False is for (issue btclib-org/btclib#814),
    and proving either on the curve here would refuse a wrong value along with a
    malformed one.
    """
    int_from_integer(r)
    int_from_integer(v)
...
def verify(
    ...
    # ValueError and EllipticCurvesRuntimeError: a well-formed opening that does
...
def _point_from_octets(octets: Octets, tag: int, name: str) -> Point:
    ...
    if prefix & 0xFE != tag ^ 1:
        raise EllipticCurvesValueError(f"not a {name}: prefix 0x{prefix:02x}")
