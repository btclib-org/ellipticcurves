...
is what turns the shared secret of :mod:`ellipticcurves.ecc.dh` into an actual
...
**Why the cipher is a parameter.** This package has no cryptographic
dependency:
...
inside this package is the worse answer rather than the cheaper one: a
...
So this module implements the half that is this package's business -- the key
...
- ``encrypt_f(key, iv, plaintext)`` returns AES-128-CBC ciphertext with
  ...
  this package's to add: it is what makes the result a whole number of 16-byte
...
is its implementations, of which Electrum's is the most read. This module is
written against `ecies_encrypt_message` and `ecies_decrypt_message` in
electrum/crypto.py and matches them byte for byte, and the test suite decrypts
ciphertexts taken from Electrum's own test vectors. That is also why nothing
here is parameterized by curve or hash function, unlike the rest of
:mod:`ellipticcurves.ecc`: every implementation that exists is secp256k1 with
sha512 and HMAC-SHA256, so a parameter would advertise an interoperability that
has no other end.
...
from ellipticcurves._utils import assert_type, bytes_from_octets, str_from_string
from ellipticcurves.alias import CipherF, Integer, Octets, String
...
from ellipticcurves.curves.sec_point import _mult_sec, _sec_from_pub_key
...
def derive_keys(prv_key: Integer, pub_key: PubKey) -> tuple[bytes, bytes, bytes]:
    ...
    :mod:`ellipticcurves.ecc.dh`'s verdict for every ECDH-shaped computation
    here: `ecdh.shared_secret` of the bindings hashes with SHA256 and BIE1 wants
    sha512 cut three ways.

    No infinity check on the shared point, unlike
    :func:`ellipticcurves.ecc.dh.diffie_hellman`, which takes a bare int: the
    ...
    # `_mult_sec` is the multiplication without the point in between.
    ...
    # calls (issue btclib-org/btclib#887)
    sec = _sec_from_pub_key(pub_key, secp256k1)
    shared_point_bytes = bytes_from_point(_mult_sec(sec, q, secp256k1), compressed=True)
...
class Envelope:
    ...
    def __init__(
        ...
        *,
        ...
        # `Octets` and the coercion an octet field takes in its
        # constructor. Assigned as they came, a value with no `len` would
        # leave `assert_valid` as a TypeError about a builtin, and one with
        # a `len` would be reported as a *size*: a two-character str is not
        # a two-byte magic, it is not a magic at all. Hex is what a caller
    ...
    def assert_valid(self) -> None:
        ...
        if len(self.magic) != _MAGIC_SIZE:
            raise EllipticCurvesValueError(
                f"invalid magic size: {len(self.magic)} bytes"
            )
        ...
        if len(self.mac) != _MAC_SIZE:
            raise EllipticCurvesValueError(f"invalid MAC size: {len(self.mac)} bytes")
    ...
    def b64decode(
        ...
        # b64decode discards whatever is not in the base64 alphabet, which would
        # make one envelope reachable from unboundedly many strings;
        # validate=True rejects that junk, and requiring the canonical
        # re-encoding covers what validate leaves to the interpreter -- the
        # reasoning is the one any strict base64 armor makes. The coercion
        # before the strip, as that method does it and for the reason issue
        # btclib-org/btclib#814 gives: what is neither text nor bytes reached
        # `.strip` untouched and left as an AttributeError about a missing
        # method. Stripping covers bytes as well as str: the whitespace around a
        ...
        try:
            ...
            raise EllipticCurvesValueError(f"invalid base64 encoding: {e}") from e
        if base64.b64encode(data_decoded) != data_bin:
            raise EllipticCurvesValueError("invalid base64 encoding: not canonical")
...
def decrypt(
    ...
    *,
    ...
    EllipticCurvesRuntimeError and the caller's cipher never runs.
    """
