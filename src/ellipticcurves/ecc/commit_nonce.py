...
from ellipticcurves._libsecp256k1 import keys as libsecp256k1_keys
from ellipticcurves._utils import bytes_from_octets, int_from_bits
...
from ellipticcurves.curves.curve import _libsecp256k1_serves, _tweak_add_var
from ellipticcurves.exceptions import EllipticCurvesRuntimeError
...
def commit_nonce_(
    ...
    # the delegation removes. Gated on the curve alone: hf enters only
    # the tagged hash above, computed in Python either way
    if _libsecp256k1_serves(ec, None):
        try:
            ...
            # the binding refuses is the zero this package refuses too
            raise EllipticCurvesRuntimeError(
                "failed to sign: zero tweaked nonce"
            ) from e
    ...
    if tweaked_nonce == 0:
        raise EllipticCurvesRuntimeError("failed to sign: zero tweaked nonce")
