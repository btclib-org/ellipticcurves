...
signature that verifies under Q with `ellipticcurves.ecc.ssa` and with any other
...
`ellipticcurves.ecc`. BIP327 is defined for that pair alone: the tags below, the
...
The message is of any size, as in `ellipticcurves.ecc.ssa`: BIP327 states no
...
from ellipticcurves._libsecp256k1 import musig as libsecp256k1_musig
from ellipticcurves._utils import assert_type, bytes_from_octets, is_octets
from ellipticcurves.alias import INF, Integer, Octets, Point
from ellipticcurves.curves import mult, multi_mult_var, scalar_from_prv_key, secp256k1
from ellipticcurves.curves.curve import _libsecp256k1_serves, _sum_var, _tweak_add_var
...
from ellipticcurves.hashes import tagged_hash
...
# `SessionContext._bindings_ctx`'s element types, under TYPE_CHECKING:
# `from __future__ import annotations` leaves an annotation a
...
# this package helper it called raises, verbatim BIP327 string or not
...
def _assert_octets_sequence(value: object, what: str) -> None:
    ...
    Every Octets is itself a Sequence, so passing one where the list of pubkeys,
    tweaks, nonces or partial signatures was meant would zip through its bytes
    and treat each as its own element (issue btclib-org/btclib#1405); `not
    isinstance(value, Sequence)` is what refuses everything else the
    comprehensions below would otherwise raise a bare TypeError on.
    """
    if is_octets(value) or not isinstance(value, Sequence):
        raise EllipticCurvesTypeError(f"invalid {what} type: {type(value).__name__}")
...
def _flag(is_xonly: bool) -> bool:
    ...
    `_utils.is_integer`'s policy mirrored: there a boolean is refused
...
def key_agg(pub_keys: Sequence[Octets]) -> KeyAggContext:
    ...
    # key and a point addition of this package's own between two of them: that
    ...
    if (
        ...
        raise EllipticCurvesRuntimeError("aggregate key is the infinity point")
...
def apply_tweak(
    ...
    if Q[1] == 0:
        ...
        raise EllipticCurvesValueError(_TWEAK_INF_ERR)
...
def nonce_gen_(
    ...
    is the one to call. That is this package's trailing underscore again:
...
def nonce_agg(pub_nonces: Sequence[Octets]) -> bytes:
    ...
    for j in (0, 1):
        ...
        # total crossing the boundary at every signer: the sum at
        # infinity above is a value, which is what the placeholder is
...
class SessionContext:
    ...
    # `_values` above and for the same reason: `musig_pubkey_agg` (+ its tweaks)
    # into a `KeyAggCache` and `musig_nonce_process` into a `Session` are
    # per-session, not per-signer, so building them once and reusing them for
    # every signer verified against this context is what makes delegating the
    # per-signer arithmetic a net win rather than a wash -- issue
    # btclib-org/btclib#1049's own measurement of `nonce_process` staying flat
    # regardless of how many signers a session has
...
class SessionValues:
    ...
    - L, second and pub_keys_set are what a per-signer key-aggregation
      coefficient is computed from -- issue btclib-org/btclib#1069, the same
      shape btclib-org/btclib#1045 solved for the fields above: fixed for the
      session, so computed here once rather than by every one of
      `_session_key_agg_coeff`'s 2n callers
    """
...
def session_values(session_ctx: SessionContext) -> SessionValues:
    ...
    `L`, `second` and `pub_keys_set` are computed here too, once, and not
    because deriving them needs anything above -- they are pure functions of
    `session_ctx.pub_keys` alone, the same as `key_agg`'s own `L` and `second`,
    which it likewise computes once and reuses for every key. Computed again
    here rather than threaded out of `key_agg_and_tweak` above: doing that would
    widen `KeyAggContext`, which callers outside this module already read, for a
    value only `_session_key_agg_coeff` wants. What earns them a place on
    `SessionValues` instead of a second cached field on `SessionContext` is that
    every caller of `_session_key_agg_coeff` -- `sign` and `partial_sig_verify_`
    ...
    except EllipticCurvesValueError as err:
    ...
    # the one this package's own BIP340 verifier recomputes, byte for byte and
...
def _session_key_agg_coeff(session_ctx: SessionContext, pub_key: bytes) -> int:
    # issue btclib-org/btclib#1069: `session_values` derives `L`, `second` and
    # `pub_keys_set` once per session -- the same fixed-input shape issue
    # btclib-org/btclib#1045 exploits for the curve arithmetic above -- so this
    # is an O(1) frozenset membership test and one hash of constant size, not
    # the three O(n) traversals of pub_keys (the membership test,
    # `_hash_pub_keys`'s join-and-hash and `_second_pub_key`'s scan) that would
    # otherwise run 2n times per session, once per signer from `sign` and once
    # per signer from `partial_sig_verify_`.
    #
    ...
    if pub_key not in values.pub_keys_set:
        raise EllipticCurvesValueError(_SIGNER_PK_ERR)
...
def sign(sec_nonce: bytearray, prv_key: Integer, session_ctx: SessionContext) -> bytes:
    ...
    if not 0 < k_1_ < secp256k1.n:
        raise EllipticCurvesValueError("first secnonce value is out of range.")
    if not 0 < k_2_ < secp256k1.n:
        raise EllipticCurvesValueError("second secnonce value is out of range.")
    ...
    if pk != bytes(sec_nonce[2 * _SCALAR_SIZE :]):
        raise EllipticCurvesValueError("Public key does not match nonce_gen argument")
    ...
    # secret half stays undelegated (issue btclib-org/btclib#1050)
...
def partial_sig_verify_(
    ...
    nonces itself, which is what this package's trailing underscore
    distinguishes -- whether the caller prepared the input or the
    library does it.

    Delegated to `btclib_secp256k1.musig` for secp256k1, sha256, a 32-byte
    message and a session with no adaptor (issue btclib-org/btclib#1049): the
    three point multiplications below are what a signer pays once per session
    and a verifier once per signer it checks, and are 3.7x slower here than in
    the bindings. secp256k1 and sha256 are this module's only pair (its own
    docstring), so `_libsecp256k1_serves` reduces to whether the bindings are
    installed and enabled. `musig_nonce_process` takes a fixed 32-byte `msg32`
    with no length parameter -- the shape of issue btclib-org/btclib#169 without
    the `sign_custom` that resolved it there, and BIP327's own empty-message and
    38-byte-message vectors are what keeps this Python arm live and validated
    for a message of any size. The bindings have no adaptor extension at all
    (this module's own docstring's paragraph on cross-validating against zkp),
    so a session that carries one takes the Python arm regardless of the other
    two conditions. Nothing else in this module is delegated -- `key_agg`,
    `key_sort` and `nonce_agg` measured too close to their Python cost, or run
    once per session already, to be worth a second code path.

    The signer's pubkey has to be one of the session's -- a membership test with
    no C equivalent, `musig_partial_sig_verify` answering a verdict for whatever
    pubkey it is given rather than asking whether it was ever aggregated.
    `_session_key_agg_coeff` is what checks it, and the delegated arm below
    calls it for that check alone, discarding the coefficient the bindings
    compute on their own -- *after* the delegated call, once C has parsed
    `pub_nonce` and `pub_key` without refusing them, exactly where the Python
    arm below checks it once it has computed `P` from the same two. Both arms
    therefore refuse a key that is malformed the same way -- the parse failure,
    whichever arm's parse finds it -- and a key that is well-formed but foreign
    the same other way, `_SIGNER_PK_ERR`, so which `EllipticCurvesValueError` a
    caller sees does not depend on which arm answered it.
    """
    values = session_values(session_ctx)
    ...
    if (
        ...
        try:
            ...
            # a parse failure: `btclib_secp256k1.musig`'s own docstring -- "a
            # parse failure ... tells this package nothing" -- so there is no
            # message of the bindings' own to translate, only the contract this
            # public function keeps on either arm: a pubnonce or pubkey that is
            # not a point is an EllipticCurvesValueError
            err_msg = "invalid pubnonce or pubkey"
            raise EllipticCurvesValueError(err_msg) from e
...
class PreSignature:
    ...
    (`ellipticcurves.ecc.musig2`'s own docstring says so), so there is no wire
...
def partial_sig_agg(psigs: Sequence[Octets], session_ctx: SessionContext) -> ssa.Sig:
    ...
    the x-only aggregate key with `ellipticcurves.ecc.ssa.verify_` and with
