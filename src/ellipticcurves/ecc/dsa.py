...
in a script interpreter's own flags, and in the leading-underscore
functions a test asks for it with.
...
sign-to-contract style (see ellipticcurves.ecc.commit_nonce for the tweak), and
...
from ellipticcurves._libsecp256k1 import ffi as libsecp256k1_ffi
from ellipticcurves._libsecp256k1 import recovery as libsecp256k1_recovery
...
from ellipticcurves.alias import BinaryData, HashF, Integer, JacPoint, Octets, Point
...
from ellipticcurves.curves.sec_point import _sec_from_pub_key
from ellipticcurves.ecc.commit_nonce import (
    commit_entropy_,
    commit_nonce_,
    commit_point_,
)
from ellipticcurves.ecc.rfc6979_nonce import _rfc6979_nonce_, challenge_
from ellipticcurves.exceptions import (
    EllipticCurvesRuntimeError,
    EllipticCurvesTypeError,
    EllipticCurvesValueError,
)
from ellipticcurves.hashes import _assert_valid_hf, reduce_to_hlen
from ellipticcurves.number_theory import mod_inv, mod_inv_var
...
def _der_length(n: int) -> bytes:
    ...
    Below 128 that is one octet, n itself; at 128 and above it is `0x80 | k`
    followed by the k big-endian octets of n -- CompactSize diverges from this
    at the same threshold, which is issue btclib-org/btclib#2130: no catalogued
    curve's element reaches it, and every catalogued curve but bpp512r1,
    nistp521 and secp521r1 keeps the sequence under it too.
    """
    if n < 0x80:
        return bytes([n])
...
def _parse_der_size(stream: BytesIO) -> int:
    ...
    if not prefix:
        raise EllipticCurvesValueError("not enough binary data for var_int")
    size = prefix[0]
    if size in _WIDE_SIZE_PREFIXES:
        ...
        if size < minimum:
            ...
            raise EllipticCurvesValueError(err_msg)
    if size > _MAX_SIZE:
...
def _parse_der_value(stream: BytesIO) -> bytes:
    ...
    malformed, so each is an EllipticCurvesValueError: the callers that filter
    parse failures -- psbt_in._assert_valid_partial_sigs among them --
    catch EllipticCurvesValueError alone.
    """
    size = _parse_der_size(stream)
    if size == 0:
        raise EllipticCurvesValueError("invalid DER length: zero size")
    value = stream.read(size)
    if len(value) != size:
        raise EllipticCurvesValueError("invalid DER length: not enough binary data")
    return value
...
def _deserialize_scalar(sig_data_stream: BytesIO, strict: bool) -> int:
    ...
    if strict:
        ...
        if len(scalar_bytes) > 1 and scalar_bytes[0] == 0 and scalar_bytes[1] < 0x80:
            raise EllipticCurvesValueError("invalid 'highest bit set' padding")
        if scalar_bytes[0] >= 0x80:
            raise EllipticCurvesValueError("invalid negative scalar")
...
class Sig:
    ...
    * value header, s-value size
    ...
    The size descriptors above are one octet each because BIP66 fixes the format
    to secp256k1, whose widest signature body is 70 octets -- DER's one-octet
    length form covers up to 127. `serialize` takes any `Curve`, and a sequence
    or an element reaching 128 octets writes X.690 8.1.3's long form instead:
    `0x80 | k` followed by k octets of length. `parse` binds `ec = secp256k1`
    and refuses that long form, matching Core's `IsValidSignatureEncoding`, so
    the length forms `serialize` can write are not all forms `parse` reads
    (issue btclib-org/btclib#2130).

    **The encoding is not delegated, and it is the one thing about a signature
    that is not** (issue btclib-org/btclib#911). The bindings have the same
    encoding as C -- `dsa.to_der`, `to_compact`, `normalize` and `is_low_s` --
    and four things say no:
    ...
    - and the two do not answer the same question. `secp256k1_der_parse_integer`
      treats an integer whose high bit is set as an *overflow* rather than as a
      malformed encoding: it zeroes the scalar and reports success, so a
      negative r parses to r = 0 instead of being refused. BIP66 refuses it, and
      so does the parser above. `tests/ecc/der_test.py` puts the whole malformed
      corpus to `dsa.signature_verify` and pins that one difference, the rest
      agreeing rule for rule -- which is the pairing that bites others, issues
      btclib-org/btclib#680 and btclib-org/btclib#667 being two libraries that
    ...
    def assert_valid(self) -> None:
        ...
        # candidate, and trying them is this package's arithmetic either way.
    ...
    def parse(
        ...
        *after* the sequence as well as what is in it: a byte too many is
        ...
        # and so must the stream: unchecked, bytes after the sequence would be
        # read as part of nothing and silently dropped, Sig.parse(der + b"\x01")
        # answering with the Sig of der. Core checks the whole element with one
        # size equation -- `(lenR + lenS + 7) != sig.size()` in
        # IsValidSignatureEncoding -- and a two-byte hash type is what leaves a
        # byte behind once the engine has stripped one, which is the
        # script_tests.json vector "P2PK with multi-byte hashtype, with DERSIG"
        # (issue btclib-org/btclib#129).
        #
        ...
        #
        # This is the one parser here with a flag in front of that rule,
        # and the flag is Core's: `_utils.assert_no_trailing` states the
        # rule everything else follows, where a complete octet string is
        # one whole object and nothing may follow it
        if strict and stream.read(1) != b"":
            raise EllipticCurvesValueError("trailing bytes after the DER sequence")
...
def _compact(sig: Sig) -> bytes:
    ...
    A signature is two scalars, and this is them: `Sig.serialize` writes the DER
    the wire carries, which the bindings would take apart again, dearer than
    this form both to write and to read back (issue btclib-org/btclib#922).
    Nothing is validated, the caller having a `Sig` whose `assert_valid` has run
    or whose values libsecp256k1 has just computed.
    """
...
def _sig_from_compact(compact: bytes, ec: Curve) -> Sig:
    ...
    would ask the library to prove its own output (issue btclib-org/btclib#888).
    """
    n_size = ec.n_size
...
def _sign_recoverable_(
    ...
    if r == 0:  # r≠0 required as it multiplies the public key
        raise EllipticCurvesRuntimeError("failed to sign: r = 0")
    ...
    if s == 0:  # s≠0 required as verify will need the inverse of s
        raise EllipticCurvesRuntimeError("failed to sign: s = 0")
    ...
    # check_validity=False, and the provenance is again the argument (issue
    # btclib-org/btclib#888), one implementation over: r == 0 and s == 0 are the
    # two refusals this function makes above, so both scalars are in 1..n-1 by
    # the time they get here, and r is x_K reduced mod n for a K this function
    # multiplied -- the congruence a validation would ask about is what r was
    # built from.
    #
...
def _grind_low_r(attempt: Callable[[int], Sig], grind: bool, ec: Curve) -> Sig:
    ...
    this package walks where that arm cannot be taken, which `sign_`'s own
...
def _python_verified(
    ...
    except (ValueError, EllipticCurvesRuntimeError):
        return False
    return True
...
def _abort_unless_checked(
    ...
    Raises:
        EllipticCurvesValueError: if the signature verifies under the private
            key's own public key and not under the one given.
        EllipticCurvesRuntimeError: if it verifies under neither, which is the
            fault this check is for.
    """
    if given is not None:
        ...
        if verified(derive()):
            raise EllipticCurvesValueError(
                "the public key given is not this private key's"
            )
        raise EllipticCurvesRuntimeError(
...
def _libsecp256k1_sign_(
    ...
    *,
    ...
    # same sequence, so a loop here would be this package re-deriving what the
    # bindings already do -- and would pay this package's own crossing per
    ...
    # and not the signing done once. What this package's own crossing is worth
    ...
    # place.
    #
    # check_validity=False, and here the provenance is the argument (issue
    # btclib-org/btclib#888): what `Sig.parse` would validate is r in 1..n-1, s
    # in 1..n-1 and r congruent to a valid x-coordinate, of the very values
    # secp256k1_ecdsa_sign has just computed -- r is the x of the nonce point
    # reduced mod n and s is a scalar beside it, so neither can be out of range
    # and an r it produced cannot fail a congruence it was built from.
    #
    ...
    # `_sig_from_compact` costs to read (issue btclib-org/btclib#922).
    #
    # A key the caller supplied crosses as they hold it, compressed or not,
    # rather than being brought to the cheaper encoding first: what makes 02||x
    # dear to parse is the field square root that recovers y, and that is the
    # same square root re-encoding it would have to do above the `try`, which
    # holds the foreign call and nothing else: a `EllipticCurvesValueError` this
    # conversion raises -- a tuple that is not on the curve, octets with the
    # wrong prefix -- would otherwise be caught as a ValueError and re-raised as
    # a new one, this package translating its own exception into itself and
    # blaming the library for it in the `__cause__` chain
    ...
    try:
        ...
        # the discrimination is theirs, but the hierarchy has to be this
        # package's: a caller catching EllipticCurvesValueError should not have
        # to know which arm answered. Their two ValueErrors are told apart by
        # asking this package's own question rather than by reading their
        # wording -- octets that are no point are octets `point_from_pub_key`
        # cannot parse either, and a key that parses was simply not this private
        # key's. That is the field square root declined above, paid only on a
        # branch that has already failed and is about to raise, which is
        # `_abort_unless_checked`'s rule one level up. Both arms then say `not a
        # public key`, and `test_the_two_arms_answer_the_same_refusals` holds
        # them to it
        if pub_key is not None:
            try:
                point_from_pub_key(pub_key, ec)
            except EllipticCurvesValueError:
                raise EllipticCurvesValueError("not a public key") from e
        raise EllipticCurvesValueError(str(e)) from e
    except (
        RuntimeError
        ...
        raise EllipticCurvesRuntimeError(str(e)) from e
...
def sign_(
    ...
    *,
    ...
    there and here, so that a signature made here is the one Core would have
    ...
    ellipticcurves.ecc.commit_nonce). Keyword-only, and the only argument that
    ...
    # whether it is true -- tests/bool_parameter_test.py is where that
    # division is kept
    ...
    if pub_key is not None and not verify:
        raise EllipticCurvesValueError(
            "pub_key is for the check that verify=False declines"
        )
    ...
    if commit_hash is not None and nonce is not None:
        raise EllipticCurvesValueError("a commitment derives its own nonce")
    ...
    if grind and (nonce is not None or commit_hash is not None):
        raise EllipticCurvesValueError("grinding derives its own nonce")
...
class Signer:
    ...
    `sign_` derives the public key, when `verify` asks for the check -- or the
    bindings do, on their own arm -- and parses it again where the check is a
    compressed key's, throwing both away once the call returns. This holds one
    across calls instead: `mult` once at construction, the generator
    multiplication `gen_keys` pays, and on the delegated arm the SEC encoding of
    it besides, so that `_delegated_sign_` hands the bindings octets rather than
    a point to re-derive. Both are read and never recomputed, which is the floor
    issue btclib-org/btclib#982 measured and this class is built to reach: a
    signature is the arithmetic of `_sign_` or one call into the bindings either
    way, and the check is what a held key removes from it.
    ...
    **This hands the caller the lifetime of a secret on both arms.**
    ...
    def sign_(
        ...
        """
        if self._wiped:
            raise EllipticCurvesValueError("the signer is wiped")
...
def _assert_as_valid_(
    ...
    *,
    ...
    if lower_s and s > ec.n // 2:
        raise EllipticCurvesValueError("not a low s")
    ...
    #
    # This arm names which check failed, "invalid (INF) key" here and "signature
    # verification failed" below, where the delegated arm at assert_as_valid_'s
    # libsecp256k1_dsa.verify call always says the latter -- it has a bool to
    # work with and this function has the arithmetic. Left unequal on purpose
    # (issue btclib-org/btclib#998, ssa.py's own `_assert_as_valid_` reading the
    # same for the reason stated there)
    if KJ[2] == 0:  # 5
        err_msg = "invalid (INF) key"
        raise EllipticCurvesRuntimeError(err_msg)
...
def _assert_commitment_(
    ...
    # the two come together or not at all, and an EllipticCurvesTypeError says
    # so because verify answers False for anything that is merely an invalid
    # signature: a commitment nobody can open, or a receipt for a commitment
    # that was never named, is a caller error and has to be heard as one
    if commit_hash is None:
        if receipt is not None:
            raise EllipticCurvesTypeError("receipt without the commitment it opens")
    elif receipt is None:
        raise EllipticCurvesTypeError("commitment without the receipt that opens it")
    else:
        ...
        if sig.r != W[0] % sig.ec.n:
            raise EllipticCurvesRuntimeError("commitment verification failed")
...
def assert_as_valid_(
    ...
    *,
    ...
    and to a script interpreter, which reads it off its own flags.
    """
    ...
    # in -- a check that proves nothing about the signer (issue
    # btclib-org/btclib#1188). The narrowing is not just an annotation:
    # the helpers called below reject the text spelling of a private key
    # too, which a PubKey annotation cannot rule out, it being a string.
    #
    # A `Sig` handed in is validated again, and this is not the redundancy
    # it looks like (issue btclib-org/btclib#888). The class is frozen, so
    # an instance that validated at construction stays valid -- but
    # `check_validity=False` says "not checked yet" and is a spelling the
    # package itself uses (`sign_` above, for values libsecp256k1 has just
    # computed), and `object.__setattr__` reaches past frozen, which the
    # suite does on purpose. What rests on this call is `verify_`'s
    # contract of answering a boolean: `Sig(-1, 5, check_validity=False)`
    # reaches `_serialize_scalar` below, where `to_bytes(...,
    # signed=False)` raises OverflowError -- an ArithmeticError, so not in
    # the (ValueError, EllipticCurvesRuntimeError) `verify_` catches, and
    # it would leave through a function whose whole answer is True or
    # False
    ...
    if _libsecp256k1_serves(sig.ec, hf):
        ...
        # validating a public key is parsing it, for a compressed one a field
        # square root, and doing it here as well lifts the same x twice -- a
        # tenth of a verification for nothing (issue btclib-org/btclib#887). So
        # a key that is no point leaves through the ValueError caught below
        sec = _sec_from_pub_key(key, sig.ec)
        # the compact form, which is the signature: `Sig.serialize` would write
        # the DER the wire carries for a call whose first act is to take it
        # apart again, an order of magnitude over what the compact form costs
        # (issue btclib-org/btclib#922). It also validated, and assert_valid has
        # just run above -- on the Sig handed in, or inside the Sig.parse that
        # made one -- so what that would run again is the congruence check of r,
        # not free even delegated and many times more where it is not
        ...
        # signature_normalize (issue btclib-org/btclib#889)
        try:
            ...
            raise EllipticCurvesValueError("not a public key") from e
        if not verified:
            # one fixed sentence, as ssa.assert_as_valid_'s own delegated arm
            # raises: libsecp256k1 answers a bool and this is this package's
            # wording for it, where the Python arm below can name which check
            # failed. Diagnostic, not API -- issue btclib-org/btclib#998 reads
            # dsa and ssa as the same question and answers both by leaving the
...
def _assert_structurally_valid_(
    ...
    equation -- into False (issue btclib-org/btclib#2170).
    ...
    *encoding* is malformed -- the file's `InvalidEncoding`,
    ...
    False (issue btclib-org/btclib#814).
    ...
    this library owes an `EllipticCurvesTypeError`.
...
def verify_(
    ...
    *,
    ...
    # ValueError and EllipticCurvesRuntimeError, not Exception: a well-formed
    ...
    # does not verify.
    #
    # The line is the annotation, and not which built-in a helper happens to
    # derive from: those two coincide only by accident, which is what issue
    # btclib-org/btclib#745 found, and issue btclib-org/btclib#814 is where the
    # annotation was chosen over the accident. CONTRIBUTING.md's "Every public
    # function validates its inputs" states the rule for the library.
    #
    # EllipticCurvesRuntimeError by name and not RuntimeError, because
...
def verify(
    ...
    *,
    ...
    # `assert_as_valid` and not a delegation to the prepared spelling: the
    # reduction has to be inside the try, or a message that is no octets is
    # refused here where the hash spelling answers False about it (issue
    # btclib-org/btclib#814) -- which is why no msg_hash is handed to the
    # structural check above, the argument this spelling declares being a
    # message rather than a digest of a stated size
    try:
...
def _recover_pub_keys_(
    ...
    # dropped candidates included, which is what those tests assert of
    # both paths.
    #
...
def recover_pub_keys_(
    ...
    # the enumeration is this package's loop and not a function libsecp256k1
    ...
    # under the Python enumeration (issue btclib-org/btclib#286). A recid is two
    ...
    if _libsecp256k1_serves(sig.ec, hf):
        ...
        for key_id in range(4 if sig.r + sig.ec.n < sig.ec.p else 2):
            ...
            # arrives as the EllipticCurvesValueError _libsecp256k1_recover_sec_
            # maps it to
            with contextlib.suppress(
                EllipticCurvesValueError, EllipticCurvesRuntimeError
            ):
                keys.append(
...
def _recover_pub_key_(
    ...
    # whether step 1.6.2 has anything left to check, which is a property of the
    # curve and not of the signature (issue btclib-org/btclib#890). With
    # cofactor 1 the curve has ec.n points and every one of them is a multiple
    # of G, so the scalars below -- reduced modulo ec.n, as scalars are -- act
    # on a lifted K the way SEC 1's arithmetic says they do, and the two
    # comparisons this enables are argued for where they are made. Above 1 a
    # lift need not land in the prime-order subgroup, the reduction is then not
    # a no-op, and the verification is doing real work: secp112r2 has cofactor
    # 4, and its recovery answers four keys of ten candidates
    ...
    if prime_order:
        # screened rather than reduced, which is the screen libsecp256k1 imposes
        # on its own side -- `recover_pub_key_`'s comment states it, "the
        # bindings require r < ec.p - ec.n for j = 1" -- and half of what leaves
        # step 1.6.2 a comparison. A `% ec.p` answers x_K = r + j*ec.n - m*ec.p
        # for some m >= 1 instead, and no such candidate can pass 1.6.2: that
        # step asks x_K % ec.n == r, which would need m*ec.p ≡ 0 mod ec.n, and
        # ec.n divides neither ec.p nor an m this small. So a wrapped x_K is
        # refused here by one comparison, where the reduction spent a Python
        # square root and a double multiplication to refuse it -- the trade
        # `_is_x_coordinate_var`'s docstring exists not to make, reached by a
        # caller who could have skipped the question (issue
        # btclib-org/btclib#891).
        #
    ...
    # the congruence holds and the INF would be reported as a recovered key,
    # (5, 0) once converted -- this package's sentinel for infinity and no key
    # at all. Q is INF whenever s*K == c*G, which is no signature anybody
    # made and is a `Sig` a caller may still hand in.
    # `ssa._recover_pub_key_` tests its own, for this reason
    if QJ[2] == 0:
        err_msg = "invalid (INF) key"
        raise EllipticCurvesRuntimeError(err_msg)
    ...
    if prime_order:
        ...
        # `_libsecp256k1_recover_sec_`'s comment below is where that argument is
        # written out: this is the arm it does not cover. What the
        # multiplication would also buy is a cross-check of 1.6.1 -- issue
        # btclib-org/btclib#183 was two implementations disagreeing while each
        # held its own copy of this arithmetic -- and that guarantee is the
        # suite's, where it already lives: the bindings are the authority this
        # path is held against, candidate by candidate and key_id by key_id
        # (issue btclib-org/btclib#890)
        return QJ
...
def _libsecp256k1_recover_sec_(
    ...
    # keys. It answers sec octets rather than a point, so a caller hashing
    # the key hashes these very bytes and never builds a point.
    #
    ...
    # `_recover_pub_key_` makes on its own side, the screen there
    ...
    try:
        ...
        # a bare ValueError is not what the Python path raises for a candidate
        # that recovers nothing -- EllipticCurvesValueError when x_K misses the
        # curve, EllipticCurvesRuntimeError when the key it yields does not
        # verify -- and a caller walking the key_ids suppresses those two by
        # name, which a ValueError does not answer to, being their base and not
        # a subclass
        raise EllipticCurvesValueError(f"invalid key_id or signature: {e}") from e
...
def _libsecp256k1_recover_point_(
    ...
    # a caller hashing the key wants the octets and never builds the
    # point at all.
...
def _recovery_inputs(
    key_id: int, msg_hash: Octets, sig: Sig | Octets, hf: HashF
) -> tuple[bytes, Sig]:
    """Validate what a recovery of one named candidate is handed."""
    # prepared is not unchecked, `is_integer`'s policy reaching this bare int
    # the way `int_from_integer` reaches every coerced one (issue
    # btclib-org/btclib#1248, CONTRIBUTING.md's "This repository in particular")
    if not is_integer(key_id):
        raise EllipticCurvesTypeError(f"non-integer key_id: {key_id}")
    ...
    # The message msg_hash: a hf_len array
    hf_len = hf().digest_size
    return bytes_from_octets(msg_hash, hf_len), sig
...
def recover_pub_key_(
    ...
    """
    msg_hash, sig = _recovery_inputs(key_id, msg_hash, sig, hf)
    ...
    # Where both answer, they answer the same, and by the same test:
...
def recover_sec_(
    ...
    *,
    ...
    """ECDSA public key recovery, answered as SEC 1 v.2 octets.

    The key `recover_pub_key_` recovers, written as `bytes_from_point`
    writes it: the form a caller hashing the key wants, which is how a
    message signature is checked against an address. Where the bindings
    serve, they write these octets themselves and no point is built.
    """
    msg_hash, sig = _recovery_inputs(key_id, msg_hash, sig, hf)
    assert_type(compressed, bool, "compressed")

    # the same guard as `recover_pub_key_`'s, whose comment says why
    if _libsecp256k1_serves(sig.ec, hf) and 0 <= key_id <= 3:
        return _libsecp256k1_recover_sec_(
            key_id, msg_hash, sig, compressed, lower_s=False
        )

    c = challenge_(msg_hash, sig.ec, hf)
    QJ = _recover_pub_key_(key_id, c, sig.r, sig.s, sig.ec, lower_s=False)
    return bytes_from_point(sig.ec.aff_from_jac_var(QJ), sig.ec, compressed)
...
def recover_sec(
    ...
    *,
    ...
    `recover_sec_` for a caller holding the message rather than its hash.
    """
    msg_hash = reduce_to_hlen(msg, hf)
    return recover_sec_(key_id, msg_hash, sig, hf, compressed=compressed)
...
def crack_prv_key_var_(
    ...
    ec = sig2.ec
    if sig1.ec != ec:
        raise EllipticCurvesValueError("not the same curve in signatures")
    if sig1.r != sig2.r:
        raise EllipticCurvesValueError("not the same r in signatures")
    if sig1.s == sig2.s:
        raise EllipticCurvesValueError("identical signatures")
