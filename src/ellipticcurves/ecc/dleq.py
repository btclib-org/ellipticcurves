...
the curve or the hash function, as in `ellipticcurves.ecc.musig2`: BIP374 is
...
`ellipticcurves.ecc.ssa`. It binds the proof to a statement of the protocol
...
from ellipticcurves._utils import bytes_from_octets
...
def generate_proof(
    ...
    An EllipticCurvesValueError for an `a` outside 1..n-1 or a B, G that is no
    ...
    if k == 0:  # pragma: no cover -- k is zero only by an unreachable hash preimage
        ...
        # signs with it
        raise EllipticCurvesRuntimeError("invalid zero nonce")
    ...
    if not verify_proof(
        ...
        raise EllipticCurvesRuntimeError("implausible proof failure")
    return proof
...
def assert_proof_as_valid(
    ...
    if s >= secp256k1.n:
        raise EllipticCurvesValueError("s not in 0..n-1")

    R1 = double_mult_var(s, G_point, -e, A_point)
    if R1[1] == 0:
        raise EllipticCurvesValueError("invalid (INF) R1")
    R2 = double_mult_var(s, B_point, -e, C_point)
    if R2[1] == 0:
        raise EllipticCurvesValueError("invalid (INF) R2")

    if e != _challenge(A_point, B_point, C_point, R1, R2, G_point, m):
        raise EllipticCurvesValueError("invalid challenge")
