...
index and the position each as 4 bytes big-endian. Signatures
btclib-org/btclib's borromean module produced before the fix of issue
btclib-org/btclib#1070, which found the message and the point swapped, do
not verify here and never will -- there is no version byte in the wire
format to switch on, `sign` and `assert_as_valid` compute this hash the
same way every time, and btclib-org/btclib's RELEASE_NOTES.md has the
"before" and "after" this cost in its breaking-changes list.

`e0` is `hf(r_0 || ... || r_last || m)`, the order `secp256k1_borromean_sign`
writes its own `sha256_e0` in: each ring's last point as the ring loop closes
that ring, the message once the loop ends, and `secp256k1_borromean_verify_impl`
recomputes it the same way (issue btclib-org/btclib#1940). No ring a verifier is
handed reaches it -- `e0` is written into the signature before any verifier sees
it -- so a signature carrying an `e0` over any other order is refused here and
there alike, and btclib-org/btclib's RELEASE_NOTES.md has what settling on
this one cost in its breaking-changes list.
...
`BorromeanSig.serialize` follows zkp's `rangeproof` module's own layout for this
signature: `e0` followed by every `s`, `ec.n_size` bytes each, ring-major, no
other framing -- generalizing zkp's hardcoded 32 to `ec.n_size` (issue
btclib-org/btclib#183) rather than contradicting it, the two agreeing wherever
`ec` is secp256k1, zkp's only curve. `BorromeanSig.parse` reads that layout back
at secp256k1 and sha256 always, as `ssa.Sig.parse` reads BIP340's one curve: the
serialization does not name either, so a `BorromeanSig` on another curve or hash
...
meets the equation that closes it (issue btclib-org/btclib#1895).
...
from ellipticcurves.alias import BinaryData, HashF, Integer, Octets, Point
...
from ellipticcurves.curves.curve import _assert_valid_ec
...
# no curve but secp256k1 can sign at all (issue btclib-org/btclib#183)


def _hash(m: bytes, R: bytes, i: int, j: int, hf: HashF) -> bytes:
    # R (the nonce point, or e0 closing the rings) before m, matching zkp's
    # secp256k1_borromean_hash order of e || m || ring || pos: swapping the two
    # produces a byte-for-byte different preimage over identical inputs (issue
    # btclib-org/btclib#1070), so this exact order is a hard break with no
    # migration -- every signature this module produces is bound to it, and one
    # made under the swapped order does not verify
    temp = b"".join(
...
def _bytes_from_ring_point(Q: Point, ec: Curve, ring: int, position: int) -> bytes:
    ...
    # EllipticCurvesValueError with neither
    try:
        return bytes_from_point(Q, ec)
    except EllipticCurvesValueError as exc:
...
def _get_msg_format(
    ...
    # preimage this one could diverge from (issue btclib-org/btclib#1070)
    t = b"".join(
...
class BorromeanSig:
    ...
    def assert_valid(self) -> None:
        ...
        if not self.s:
            raise EllipticCurvesValueError("no rings")

        for i, ring in enumerate(self.s):
            # a ring with no keys proves nothing about any key, the same
            # degeneracy as no rings at all, one level down: left unrefused
            # here, `assert_as_valid` indexed e[i][0] on the empty list
            # `_initialize` builds for it and a bare IndexError escaped where
            # `verify` is documented to answer `False` (issue
            # btclib-org/btclib#1094). Checked here rather than in
            # `assert_as_valid` or `_assert_matches_pubk_rings` because it holds
            # for `self.s` alone, independent of whatever `pubk_rings` a caller
            # later checks it against -- and this way `BorromeanSig(e0, [[]],
            # ec)` is already refused at construction, check_validity's default
            # being True
            if not ring:
                raise EllipticCurvesValueError(f"ring {i} has no keys")
    ...
    def serialize(self, *, check_validity: bool = True) -> bytes:
        ...
        `secp256k1_scalar_get_b32` per public key in ring order. `ec.n_size`
        generalizes zkp's hardcoded 32 (issue btclib-org/btclib#183) rather than
        contradicting it -- the two agree wherever `ec` is secp256k1, zkp's only
        curve.
        """
        if check_validity:
            self.assert_valid()
...
def _assert_matches_pubk_rings(
    ...
    `sign` and `assert_as_valid` both walk `s[i][j]` against `pubk_rings[i][j]`
    -- `_initialize` is where the two already meet, building `e` in that same
    shape from `pubk_rings` alone. Unless `s` describes the identical shape, one
    ring per pubkey ring and one value per key, that walk runs past the end of a
    ring's tuple and a bare `IndexError` escapes where `verify` is documented to
    answer `False` (issue btclib-org/btclib#1088). Checked once, here, before
    the walk -- not as each index into `s` is taken, which is what let the
    `IndexError` through in the first place.
...
def _assert_sign_key_idx_in_range(
    ...
    Nothing bounded what a value of `sign_key_idx` pointed at: `sign`'s own
    length check only compares the *counts* of `pubk_rings`, `sign_key_idx`,
    `ks` and `sign_keys`, one entry per ring on all four, and says nothing about
    what a value inside `sign_key_idx` names. Unchecked, step 1's `(j_star + 1)
    % keys_size` is a ZeroDivisionError for a ring with no keys (issue
    btclib-org/btclib#1094, on the signing side -- `BorromeanSig.assert_valid`'s
    own check cannot reach this walk, which runs before any `BorromeanSig` is
    built), and step 2's `range(1, j_star + 1)` walks past the end of a ring's
    tuple for a value at or past its size (issue btclib-org/btclib#1095); a
    negative value does not raise at all, Python's own negative indexing quietly
    reading a different position than the one named. `0 <= sign_key_idx[i] <
    len(pubk_rings[i])` refuses all three, checked here once before either step.
    """
...
def sign_(
    ...
    # one entry per ring in each of the four, checked here rather than left to
    # the `strict=True` of the two loops below: a short ks would truncate them
    # silently and sign a subset of the rings -- a signature over fewer rings
    # than the caller asked for, which is the one thing a ring signature must
    # not do quietly. zip's own message is a EllipticCurvesValueError's class
    # with none of its content, naming "argument 3" and no parameter of this
    # function; strict=True stays, as the assertion that this check and those
    # loops cannot drift apart. sign_keys is part of this check for the same
    # reason as the other three: step 2 indexes it per ring, through the
    # `q_ints` built from it one entry at a time, so a shorter sequence there is
    # the same shape exposure _assert_matches_pubk_rings refuses on the
    # verification side, one ring's worth of tuple away from a bare IndexError
    ...
    # nothing checked it was one before step 1 read it -- see the docstring
    # above for the two ways that went wrong (issues btclib-org/btclib#1094,
    # btclib-org/btclib#1095). Checked once here, before either step, the same
    # shape as the length check just above and btclib-org/btclib#1088's decision
    ...
    # `sign_keys[i]` as it came, so a key of 0, of n, or of q + n signed where
    # every other signer in `ecc` refuses all three -- q + n silently the same
    # key as q, the reduction being that line's own `% ec.n` -- and the octets
    # and hex spellings `Integer` names left Python's `OverflowError` for a key
    # and its `TypeError` for a nonce, neither of them anything this library
    # declares (issue btclib-org/btclib#1243). Read before step 1 rather than in
    # the loop that consumes each, so a scalar a later ring's would have refused
    # costs no scalar multiplication first.
    #
    ...
    for i, (pubk_ring, j_star, k) in enumerate(
        ...
        if start_idx != 0:
            for j in range(start_idx, keys_size):
                ...
                # e is already reduced mod n, so only zero can trip this, and
                # for secp256k1 with sha256 zero is a 2**-255 accident: exactly
                # two of the 256-bit outputs (0 and n) are 0 mod n. On a
                # low-cardinality curve it is one message in n -- one in eleven
                # on ec13_11 -- which is the corner case issue
                # btclib-org/btclib#183 asked for and what makes ec a parameter
                # worth having: tests/ecc/borromean_test.py reaches this raise,
                # and the three below it, on that curve
                if not 0 < e[i][j] < ec.n:
    ...
    # after the ring loop, not before it (issue btclib-org/btclib#1940)
    e0bytes += m
    hasher = hf()
...
def sign(
    ...
    `ks` and `sign_keys` are scalars, spelled as `Integer` the way `dsa` and
    `ssa` spell one, and each is read through `curves.scalar_from_prv_key`: in
    1..n-1, or refused (issue btclib-org/btclib#1243). `sign_key_idx` is not one
    of them and stays `int` -- it indexes a ring, and an index is not a scalar
    written in hex. `sign_key_idx[i]` must be a valid index into
    `pubk_rings[i]`, refused with `EllipticCurvesValueError` naming the ring,
    the index and the ring's size otherwise -- a ring with no keys has none,
    whatever the index (issue btclib-org/btclib#1094), and a value the ring's
    size does not reach either (issue btclib-org/btclib#1095).
...
def _assert_structurally_valid_(
    ...
    does not close on `sig.e0` -- into False (issue btclib-org/btclib#2170).
...
def verify(
    ...
    # ValueError and EllipticCurvesRuntimeError: a well-formed signature whose
...
def assert_as_valid(
    ...
    The rings are walked forward from `sig.e0` and must close on the `e0` they
    started from; errors carry the reason and which ring and position it
    happened at (`BorromeanRingError`), `verify` being the boolean answer. A
    `sig` whose ring shape disagrees with `pubk_rings` -- a different number of
    rings, or a ring with a different number of s-values than it has keys -- is
    refused first, as a plain `EllipticCurvesValueError` naming the ring and the
    counts: two arguments that do not describe the same object is not a check
    that ran and failed, so it is not a `BorromeanRingError` (issue
    btclib-org/btclib#1088). A `sig` carrying a ring with no keys is refused
    too, by `sig.assert_valid` below rather than here -- it proves nothing about
    any key regardless of what `pubk_rings` says, so `BorromeanSig`'s own
    invariant is what refuses it (issue btclib-org/btclib#1094).
    ...
    # pubk_ring[j] without ever re-checking it (issue btclib-org/btclib#1088)
