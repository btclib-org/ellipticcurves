...
"""The Confidential Transactions rangeproof, and what this package writes of it.
...
from ellipticcurves.alias import INF, BinaryData, Integer, Octets, Point
...
from ellipticcurves.ecc.borromean import BorromeanSig, PubkeyRing, _hash
...
from ellipticcurves.ecc.rfc6979_nonce import _HmacDrbg
...
def _assert_valid_mantissa(mantissa: int) -> None:
    ...
    if not 1 <= mantissa <= _MAX_MANTISSA:
        raise EllipticCurvesValueError(f"rangeproof mantissa not in 1..64: {mantissa}")
...
def _assert_valid_header(exp: int, mantissa: int, min_value: int | None) -> None:
    ...
    if mantissa:
        if not 0 <= exp <= _MAX_EXP:
            raise EllipticCurvesValueError(f"rangeproof exponent not in 0..18: {exp}")
    elif exp != -1:
    ...
    for i, size in enumerate(rsizes):
        ...
        for j in range(size):
            ...
            if not 0 < s < secp256k1.n:
                raise EllipticCurvesRuntimeError("rangeproof nonce is not a scalar")
            ring.append(s)
...
class RangeProof:
    ...
    def parse(
        ...
        if flags & _RESERVED:
            raise EllipticCurvesValueError("bit 7 set in the rangeproof flags")
        exp, mantissa = -1, 0
        ...
        if padding and octets[-1] >> padding:
            raise EllipticCurvesValueError("rangeproof sign bit padding is not zero")
...
def _challenge(m: bytes, r: bytes, i: int, j: int) -> int:
    ...
    if not 0 < e < secp256k1.n:
        raise EllipticCurvesRuntimeError("rangeproof challenge is not a scalar")
    return e
...
def _borromean_sign(
    ...
    for i, (ring, k, j_star) in enumerate(
        ...
        if s[i][j_star] == 0:
            ...
            raise EllipticCurvesRuntimeError("rangeproof signature value is zero")
...
def sign(
    ...
    *,
    ...
    if not 0 <= value <= _UINT64_MAX:
        raise EllipticCurvesValueError(f"rangeproof value not in 0..2**64-1: {value}")
    if not 0 <= min_value <= value:
        err_msg = f"rangeproof min value not in 0..{value}: {min_value}"
        raise EllipticCurvesValueError(err_msg)
    if not -1 <= exp <= _MAX_EXP:
        raise EllipticCurvesValueError(f"rangeproof exponent not in -1..18: {exp}")
    if not 0 <= min_bits <= _MAX_MANTISSA:
        raise EllipticCurvesValueError(f"rangeproof min bits not in 0..64: {min_bits}")
    ...
    if secs[-1] == 0:
        raise EllipticCurvesRuntimeError("rangeproof last ring blinding factor is zero")
...
def _borromean_verify(
    ...
    if sha256(e0_preimage + m).digest() != sig.e0:
        raise EllipticCurvesRuntimeError("rangeproof signature verification failed")
...
def _proof_from(proof: RangeProof | Octets) -> RangeProof:
    ...
    asked it here, as `borromean` and `ssa` ask their own object
...
def _assert_structurally_valid_(proof: RangeProof | Octets) -> RangeProof:
    ...
    False (issue btclib-org/btclib#2170).
    ...
    `borromean` and `ssa` have: `_proof_from` asks a `RangeProof`
    argument `assert_valid`, so what `assert_as_valid` repeats below is
    that check and not this parse.

    `commitment`, `gen` and `extra_commit` are asked nothing here. The first two
    enter as native points that no parse reads -- `ecc.pedersen.commit` and
    `ecc.pedersen.second_generator` are where each is built -- and
    `extra_commit` is what the proof is written *about*, on issue
    btclib-org/btclib#814's side of the line with `ecc.dsa`'s and `ecc.ssa`'s
    messages.
    """
...
def verify(
    ...
    *,
    ...
    asked is `assert_valid` and a state that refuses is False here and an
    `EllipticCurvesValueError` from `assert_as_valid`. See `_proof_from`.
    """
    parsed_proof = _assert_structurally_valid_(proof)
    # ValueError and EllipticCurvesRuntimeError: a well-formed proof that does
...
def rewind(
    ...
    *,
    ...
    if rsizes == (1,):
        ...
        if found is None:
            raise EllipticCurvesRuntimeError(
                "rangeproof rewind reads no value encoding"
            )
        skip1, v = found
        skip2 = (v >> 2 * last) & 3
        if skip1 == skip2:
            raise EllipticCurvesRuntimeError(
                "rangeproof rewind reads the value at the digit"
            )
    ...
    if _commit(blind, value, gen, secp256k1) != commitment:
        raise EllipticCurvesRuntimeError(
            "rangeproof rewind does not open the commitment"
        )
