...
schedule this package implements: a construction over a hash rather than over
...
**Why they are here rather than beside the agreement that uses one** (issue
btclib-org/btclib#1080). SEC 1 defines ANSI-X9.63-KDF in section 3.6.1 for the
agreement of section 6.1, so `ecc.dh` is where it reads naturally, and with one
KDF in the tree that costs nothing. HKDF has no such tie -- RFC 5869 has no
Diffie-Hellman in it, and nothing that wants HKDF here holds a public key -- so
a drawer under `ecc` leaves either a KDF reached through a package of signature
schemes or two KDFs in two places for the next one to have to choose between.
One drawer, at the layer its contents belong to, is the answer to both;
btclib-org/btclib's RELEASE_NOTES.md has the spellings it costs.
"""
...
from ellipticcurves._utils import is_integer
...
def _assert_valid_keying_data_size(size: int, max_size: int) -> None:
    ...
    """
    # the range is checked before the loop bound is computed from it, and the
    # type before the range. Unchecked, a negative size made the loop empty and
    # the final negative slice returned b"" -- an empty key where the caller
    # asked for keying material, which is the one answer a key derivation
    # function must never invent (issue btclib-org/btclib#321). A float went
    # further still and reached that slice, leaving through a bare TypeError
    # from underneath the library rather than through its own exception contract
    if not is_integer(size):
        raise EllipticCurvesTypeError(f"non-integer keying data size: {size}")
    ...
    if size <= 0:
        raise EllipticCurvesValueError(f"invalid keying data size: {size}")
    if size > max_size:
        raise EllipticCurvesValueError(
            f"cannot derive a key larger than {max_size} bytes"
        )
...
def ansi_x9_63_kdf(z: bytes, size: int, hf: HashF, shared_info: bytes | None) -> bytes:
    ...
    `size` is a positive number of octets, SEC 1's keydatalen: an
    EllipticCurvesTypeError if it is no integer, an EllipticCurvesValueError if
    it is zero, negative, or above what the hash function can derive.
...
def hkdf_expand(prk: bytes, size: int, hf: HashF, info: bytes | None) -> bytes:
    ...
    `size` is a positive number of octets, RFC 5869's L: an
    EllipticCurvesTypeError if it is no integer, an EllipticCurvesValueError if
    it is zero, negative, or above 255 digests. A `prk` below a digest is an
    EllipticCurvesValueError too.
    ...
    if len(prk) < hf_size:
        raise EllipticCurvesValueError(f"pseudorandom key shorter than {hf_size} bytes")
