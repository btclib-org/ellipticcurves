...
SEC2v2, NIST and Brainpool, and this module is where ellipticcurves.curves keeps
...
about a curve. The loader's own names are underscored, a `with` target and
...
from ellipticcurves._libsecp256k1 import ENABLED as _bindings_enabled
from ellipticcurves._libsecp256k1 import INSTALLED as _bindings_installed
...
from ellipticcurves._libsecp256k1 import (
    pubkey_tweak_add as libsecp256k1_pubkey_tweak_add,
)
from ellipticcurves._libsecp256k1 import (
...
from ellipticcurves._libsecp256k1 import xonly_to_pubkey as libsecp256k1_xonly_to_pubkey
from ellipticcurves._utils import assert_type, hex_string, int_from_integer, is_integer
from ellipticcurves.alias import INF, HashF, Integer, JacPoint, Point
...
from ellipticcurves.number_theory import legendre_symbol_var
...
def _generator_from_point(G: Point, ec: CurveGroup) -> Point:
    ...
    """
    if len(G) != 2:
        raise EllipticCurvesValueError("generator must be a sequence[int, int]")
    point = int_from_integer(G[0]), int_from_integer(G[1])
    if not ec.is_on_curve(point):
        raise EllipticCurvesValueError("Generator is not on the curve")
    return point
...
def _assert_mov_resistant(p: int, n: int) -> None:
    ...
    An EllipticCurvesValueError, as every other rejection in this constructor
    is: the embedding degree is a parameter of the curve being refused, so the
    caller that catches an unusable `n` catches this one too. The n == p check
    below is the other weak-curve refusal, and the two disagreeing on their
    class is what left this one uncatchable for anyone holding the library to
    its own exception contract.
    """
...
class Curve(CurveGroup):
    ...
    round on purpose: this is the only group anything else in this package
    multiplies in, and `Curve` is what a caller asks for.

    Two things follow from n being prime, and ellipticcurves.ecc relies on both:
    ...
    def __init__(
        ...
        # 0, never 1 (issue btclib-org/btclib#166)
        if n == self.p:
            raise EllipticCurvesValueError(f"n=p weak curve: {hex_string(n)}")
...
def _assert_valid_ec(ec: Curve) -> None:
    ...
    is what makes it `_utils.assert_type`'s, which is the one spelling of
...
# it Python.
#
# What the import answered, and not a constant: `ellipticcurves._libsecp256k1`
# is the one place the bindings are imported, so an installation without them
# starts here at False and every dispatch in the package declines. It is also
# the seam whatever wants the Python arithmetic closes to reach it -- the test
# suite included, which assigns False to it where the bindings are installed and
# serving
...
def is_libsecp256k1_serving() -> bool:
    ...
    project built on this package asks when it must not check libsecp256k1 with
    ...
    performance" -- and issue btclib-org/btclib#198 is btclib's side of it.
    """
...
def set_libsecp256k1_serving(*, serving: bool) -> None:
    ...
    `ELLIPTICCURVES_NO_LIBSECP256K1` set to a non-empty value makes the initial
...
def is_x_coordinate_var(x: int, ec: Curve = secp256k1) -> bool:
    ...
    _assert_valid_ec(ec)
    if not is_integer(x):
        raise EllipticCurvesTypeError(f"non-integer x-coordinate: {x}")
...
def _sec_from_point(Q: Point) -> bytes:
    ...
    bindings against two int.from_bytes, the lesson public BIP32 derivation
    teaches. The 32 octets more are all but free to
    write here.
    """
...
def _libsecp256k1_multi_mult_var_(
    ...
    infinity and nothing else. The alternative is a combine per term with
    a running total this side compares coordinates on, the only way of
    telling the two zeros apart were the sum assembled here.

    No term crosses either, which is the other half and the smaller one: a
    `pubkey_tweak_mul` per term would serialize its product for a
    `pubkey_sum` that parses it straight back, a seventh of what the call
    costs with them from eight terms up, a little less below it --
    including `double_mult_var`'s two, the count most callers have.
    Handing over the whole equation is what avoids it, and that is the one
    ...
    per term count is in btclib-org/btclib's CHANGELOG, in the entry that
    took each half, which is where a figure belongs: an entry is read as
    the history of a release and this is read as a statement about the
    code as it stands.
...
def _multi_mult_x_only_var(
    ...
    the one place in this package that hands the bindings many scalars at once,
    ...
    Every scalar is required to be in 1..n-1 and every x to be an x-coordinate.
    The caller is what holds to that -- ssa's rand is drawn in that range, its
    challenge is refused at zero, n is prime -- so what is left for the bindings
    to refuse is a term they cannot read, and finding which is the lift the
    accepting path does not pay. Issue btclib-org/btclib#622 made that trade the
    other way round in `Sig.assert_valid`, and it is the same one: the refusing
    path can afford a square root. The message is `ec.y_var`'s, from the lift
...
def _mult_checked(m: int, Q: Point | None, ec: Curve, *, prepared: bool) -> Point:
    ...
    # this package has, so the point it multiplies is rescaled first: same
...
class PreparedPoint:
    ...
    Both are the Python arithmetic, and what puts a call on them is that call's
    own guard: `_libsecp256k1_serves` -- the process-wide switch, which an
    installation without the bindings leaves off and which
    `set_libsecp256k1_serving` and `ELLIPTICCURVES_NO_LIBSECP256K1` turn off in
    a process that has them, and the curve -- with whatever that call ands onto
    it. Neither call passes a hash function: a verification's is spent on its
    own module's guard, and `_jac_double_mult_var` under that guard asks the
    predicate again with none, so a hash function the bindings do not serve
    leaves the multiplication delegated and the tables handed to it unused.
    Where a call's whole guard holds there is nothing here to reach --
    libsecp256k1 verifies in a fraction of either arm and holds tables of its
    own -- so handing a prepared point in on that path is not an error and costs
    nothing; it simply buys nothing.

    Preparing is a caller's word and never inferred, and the memory is why: the
    tables are per distinct point, so a library that memoized whatever public
    key arrived would hold a few MB for keys nobody will see again -- issue
    btclib-org/btclib#287, the bound `_cached_base58_decode` and
    `pedersen.second_generator` hold too. What bounds it here is that nothing is
    prepared unless asked, and beyond that the `lru_cache` maxsize the tables
    live under.
    ...
    Raises:
        EllipticCurvesValueError: if the point is not on the curve, or is
    ...
    def __init__(self, point: Point, ec: Curve = secp256k1) -> None:
        ...
        if not point[1]:
            raise EllipticCurvesValueError("cannot prepare the point at infinity")
...
def _sum_var(points: Sequence[Point], ec: Curve) -> Point:
    ...
    the signers, where one `keys.pubkey_sum` of all the terms takes the
    place of a running total crossing the boundary at every step, and is
    worth better than half of both. The measurement per term count, and at
    those two callers, is in btclib-org/btclib's CHANGELOG, in the entry
    that took it, an entry being read as the history of a release where
...
class TweakChain:
    ...
    caller, per number of outputs found, is in btclib-org/btclib's
    CHANGELOG, in the entry that took it.
    ...
    Raises:
        EllipticCurvesTypeError: if the curve is not a Curve.
        EllipticCurvesValueError: if the point is not on the curve.
    """
