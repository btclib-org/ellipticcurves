...
class CurveGroup:
    ...
    def add_jac(self, Q: JacPoint, R: JacPoint) -> JacPoint:
        ...
        # and V vanishes for an operand that is not a point at all -- P + INFJ
        # answered 2*P, and eight of the ten scalar multiplications this package
        # offers were wrong for most scalars on every p == 7 curve (issue
        # btclib-org/btclib#171). The stand-ins do not make it safe to read:
        # they are values, and V can be zero on a pair of them too, one time in
        # p
...
def _additions_by_position(
    ...
    by an addition (issue btclib-org/btclib#906).
