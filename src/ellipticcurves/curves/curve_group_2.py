...
`curve_group`'s own comments, and issue btclib-org/btclib#183 tracks the smaller
...
def _mult_sliding_window_var(m: int, Q: JacPoint, ec: CurveGroup, w: int) -> JacPoint:
    ...
    """
    if m < 0:
        raise EllipticCurvesValueError(f"negative m: {hex(m)}")

    # a number cannot be written in basis 1 (ie w=0)
    if w <= 0:
        raise EllipticCurvesValueError(f"non positive w: {w}")
...
def _double_mult_w_NAF_var(
    ...
    The gap over _double_mult_var is the wider for add_jac not
    ...
    """
    if u < 0:
        raise EllipticCurvesValueError(f"negative first coefficient: {hex(u)}")
    if v < 0:
        raise EllipticCurvesValueError(f"negative second coefficient: {hex(v)}")
    # a number cannot be written in basis 1 (ie w=0)
    if w <= 0:
        raise EllipticCurvesValueError(f"non positive w: {w}")
...
def _multiplier_decomposer(m: int) -> tuple[int, int]:
    ...
    Signed and mod n are the whole of the algorithm (issue
    btclib-org/btclib#215): the congruence only holds mod the group order, and
    secp256k1's p and n share their top 128 bits, so reducing mod ec.p instead
    would send every scalar above ~2^127 to a wrong answer. Rounding is to
    nearest, not with ceil, whose bias costs the balance: round-to-nearest is
    what makes c1, c2 the closest lattice point and m1, m2 short. The results
    stay signed, since a final mod-p reduction would throw away the sign that
    point negation is there to absorb, and hand _double_mult_var two 256-bit
    multipliers for an 8-bit m.
    """
    m %= _N
