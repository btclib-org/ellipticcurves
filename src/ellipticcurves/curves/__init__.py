...
"""Module ellipticcurves.curves.

**The arithmetic.** ellipticcurves.curves holds the elliptic curve itself: the
field and group operations of `curve_group`, the `Curve` built on them, the
catalogued curves (secp256k1 among them), and the SEC encoding of a point.
Nothing here knows what a signature is.

What is built *on* a curve lives in ellipticcurves.ecc -- dsa, ssa, borromean,
...
The two names are easy to conflate -- everything in ecc is also about curves --
so the anchor is worth stating: `from ellipticcurves.curves import mult`, `from
ellipticcurves.ecc import dsa`.
...
The other multiplications of ellipticcurves.curves.curve_group and
ellipticcurves.curves.curve_group_2 -- _mult_aff_var, _mult_jac_var,
...
one, by a factor of four: _mult_fixed_base makes no doubling at all, where a
wNAF makes one per bit however wide its table is and however thoroughly it is
cached, the factor holding over the memoized odd multiples of G at w=8, w=10 and
w=12 alike. So the arm every key derivation, every BIP32 child and every signing
nonce runs has no variable-time alternative to offer. What is left is a
variable-base mult with a secret scalar, measured over ecc.dh: 1.13x on
secp256k1 and 1.03x on nistp256, the second being that measurement's own noise.
Beside it, the blinded nonce inverse is a fraction of a percent of a signature,
the projective blinding of curve_group._blinded_jac is 1%, and every
verification is already _var. The 1.13x was measured with the bindings switched
off. btclib_secp256k1 is the secp256k1 extra rather than a required dependency,
so a caller who skips it is already in that state, with nothing to switch.
...
Beside the two point conversions the codec carries bytes_from_prv_key_int, the
composition of a multiplication and an encoding that every private-to-public
conversion is, answered for secp256k1 out of the bindings' own serialization,
without materializing the point (issue btclib-org/btclib#127); and
scalar_from_prv_key, which reads a private key the other way, as the scalar in
1..n-1 that it is. What a private key may be spelled as at this layer is the
integer, its n_size octets, or their hex. That is narrower than everything
Integer takes -- int_from_integer reads a "0x" prefix and a short hex string,
which a key of a fixed size must not be -- and the annotation is Integer because
the two are the same union of types and a second name for it would say nothing
mypy could check.
...
each carries a network and a compression flag, which are not facts about
a curve (issue btclib-org/btclib#1188).
"""
...
from ellipticcurves.curves.curve_group import CurveGroup
