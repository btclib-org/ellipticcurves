...
"""The hashing ellipticcurves.ecc's schemes do.
...
from ellipticcurves.alias import HashF, Octets
...
def _assert_valid_hf(hf: HashF) -> None:
    ...
    An `EllipticCurvesTypeError` for the reason the class exists, and with a
    consequence the boolean verifications depend on: it is a `TypeError`,
    so their `except (ValueError, EllipticCurvesRuntimeError)` does not catch it
    and a caller's mistake reaches the caller instead of being reported as
    a signature that does not verify.
    """
    if not callable(hf):
        raise EllipticCurvesTypeError(f"not a hash function: {hf!r} is not callable")
