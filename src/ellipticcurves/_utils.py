...
from ellipticcurves.alias import BinaryData, Integer, Octets, String
...
def bytes_from_octets(octets: Octets, out_size: NoneOneOrMoreInt = None) -> bytes:
    ...
    if isinstance(octets, str):  # hex string
        ...
        try:
            ...
            raise EllipticCurvesValueError(f"invalid hex string: {e}") from e
        ...
        raise EllipticCurvesTypeError(err_msg)
...
def str_from_string(s: String, what: str) -> str:
    ...
    out would fly past every caller written to catch an
    ...
    try:
        ...
        raise EllipticCurvesValueError(f"non-ascii character in {what}: {e}") from e
...
def assert_no_trailing(data: BinaryData, stream: BytesIO, what: str) -> None:
    ...
    if trailing:
        raise EllipticCurvesValueError(f"{len(trailing)} bytes after the {what}")
...
def int_from_integer(i: Integer) -> int:
    ...
    * b'\xde\xad\xbe\xef'
    ...
    if isinstance(i, int):
        ...
        # runs through
        if not is_integer(i):
            raise EllipticCurvesTypeError(f"non-integer: {i}")
        return i
    ...
    if isinstance(i, str):
        ...
        if i.startswith(("0x", "-0x")):
            ...
            try:
                ...
                raise EllipticCurvesValueError(f"invalid hex integer: {i!r}") from e
...
def hex_string(i: Integer) -> str:
    ...
    if int_ < 0:
        raise EllipticCurvesValueError(f"negative integer: {int_}")
    a_str = f"{int_:x}"
    if len(a_str) % 2 != 0:
        a_str = f"0{a_str}"
