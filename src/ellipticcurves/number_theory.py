...
from ellipticcurves._utils import hex_string, is_integer
...
def _assert_valid_operand(a: int) -> None:
    ...
    """
    if not is_integer(a):
        raise EllipticCurvesTypeError(f"not an integer: {a!r}")
...
def _assert_valid_modulus(m: int) -> None:
    ...
    zero, neither of which a caller writing `except EllipticCurvesValueError`
    ...
    if m < 1:
        raise EllipticCurvesValueError(f"non-positive modulus: {m}")
...
def mod_inv_var(a: int, m: int) -> int:
    ...
    try:
        ...
        raise EllipticCurvesValueError(err_msg) from None
