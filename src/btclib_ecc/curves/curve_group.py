...
cyclic nor of prime order. Curve, in the btclib_ecc.curves.curve module, is
...
class CurveGroup:
    ...
    group are the parameters of Curve, in btclib_ecc.curves.curve, which is
    ...
    def __init__(self, p: Integer, a: Integer, b: Integer) -> None:
        ...
        if a < 0:
            raise BTClibEccValueError(f"negative a: {a}")
        if p <= a:
            err_msg = "p <= a: " + (
            ...
            raise BTClibEccValueError(err_msg)
        if b < 0:
            raise BTClibEccValueError(f"negative b: {b}")
        if p <= b:
            err_msg = "p <= b: " + (
        ...
        d = 4 * a * a * a + 27 * b * b
        if d % p == 0:
            raise BTClibEccValueError("zero discriminant")
        self._a = a
        self._b = b
    ...
    def negate(self, Q: Point) -> Point:
        ...
        raise BTClibEccTypeError("not a point")
    ...
    def negate_jac(self, Q: JacPoint) -> JacPoint:
        ...
        raise BTClibEccTypeError("not a Jacobian point")
    ...
    def is_on_curve(self, Q: Point) -> bool:
        ...
        if len(Q) != 2:
            raise BTClibEccValueError("point must be a tuple[int, int]")
...
def _multi_mult_pairs(
    ...
    if len(scalars) < 2:
        raise BTClibEccValueError("not a multi_mult_var")
