...
def scalar_from_prv_key(prv_key: Integer, ec: Curve = secp256k1) -> int:
    ...
    if not 0 < q < ec.n:
        raise BTClibEccValueError("private key not in 1..n-1")
...
def bytes_from_point(Q: Point, ec: Curve = secp256k1, compressed: bool = True) -> bytes:
    ...
    if Q[1] == 0:  # infinity point in affine coordinates
        raise BTClibEccValueError("no bytes representation for infinity point")
...
def _assert_pub_key_type(pub_key: PubKey) -> None:
    ...
    last: a value of no key type at all is a BTClibEccTypeError here, kept
    apart from the BTClibEccValueError a spelling raises once its type is
    ...
    if not isinstance(pub_key, _PUB_KEY_TYPES):
        raise BTClibEccTypeError("not a public key")
