...
SEC2v2, NIST and Brainpool, and this module is where btclib_ecc.curves keeps
...
from btclib_ecc._libsecp256k1 import ENABLED as _bindings_enabled
from btclib_ecc._libsecp256k1 import INSTALLED as _bindings_installed
...
from btclib_ecc._libsecp256k1 import xonly_to_pubkey as libsecp256k1_xonly_to_pubkey
from btclib_ecc._utils import assert_type, hex_string, int_from_integer, is_integer
from btclib_ecc.alias import INF, HashF, Integer, JacPoint, Point
...
def _generator_from_point(G: Point, ec: CurveGroup) -> Point:
    ...
    """
    if len(G) != 2:
        raise BTClibEccValueError("generator must be a sequence[int, int]")
    point = int_from_integer(G[0]), int_from_integer(G[1])
    if not ec.is_on_curve(point):
        raise BTClibEccValueError("Generator is not on the curve")
    return point
...
class Curve(CurveGroup):
    ...
    Two things follow from n being prime, and btclib_ecc.ecc relies on both:
    ...
    def __init__(
        ...
        if n == self.p:
            raise BTClibEccValueError(f"n=p weak curve: {hex_string(n)}")
...
# it Python.
#
# What the import answered, and not a constant: `btclib_ecc._libsecp256k1`
...
def is_x_coordinate_var(x: int, ec: Curve = secp256k1) -> bool:
    ...
    _assert_valid_ec(ec)
    if not is_integer(x):
        raise BTClibEccTypeError(f"non-integer x-coordinate: {x}")
...
class PreparedPoint:
    ...
    `set_libsecp256k1_serving` and `BTCLIB_ECC_NO_LIBSECP256K1` turn off in
    ...
    Raises:
        BTClibEccValueError: if the point is not on the curve, or is
...
class TweakChain:
    ...
    Raises:
        BTClibEccTypeError: if the curve is not a Curve.
        BTClibEccValueError: if the point is not on the curve.
    """
