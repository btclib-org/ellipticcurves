...
from btclib_ecc.hashes import _assert_valid_hf, reduce_to_hlen, tagged_hash
...
def _checked_sign_(c: int, signature: Sig, x_Q: int, ec: Curve, verify: bool) -> Sig:
    ...
    try:
    ...
    except (ValueError, BTClibEccRuntimeError) as e:
        ...
        # sG - cQ is the point at infinity. `BTClibEccRuntimeError` by
...
def sign_(
    ...
    *Default Signing* -- "If Verify(bytes(P), m, sig) returns failure,
    ...
    check that fails raises BTClibEccRuntimeError saying that signing
    ...
    if _libsecp256k1_serves(ec, hf) and commit_hash is None:
        ...
        try:
            ...
            # caller catching BTClibEccException should not have to know
...
def _assert_as_valid_(
    ...
    if ec.y_aff_from_jac_var(KJ) % 2:
        raise BTClibEccRuntimeError("y_K is odd")
    ...
    *,
    ...
    if _libsecp256k1_serves(sig.ec, hf):
        ...
        if not verified:
            ...
            # BTClibEccRuntimeError, which is the contract; the message is
...
def verify_(
    ...
    *,
    ...
    # ValueError and BTClibEccRuntimeError: a well-formed signature that is
...
def _assert_batch_sequences(
    ...
    """
    if is_octets(msgs):
        raise BTClibEccTypeError(f"invalid msgs type: {type(msgs).__name__}")
...
def assert_batch_as_valid_(
    ...
    **Which leaves the question of why secp256k1 runs the equation at
    ...
    if len(sigs) != batch_size:
        raise BTClibEccValueError(_err_msg(batch_size, "signatures", sigs))
    if batch_size == 1:
    ...
    if any(sig.ec != ec for sig in sigs):
        raise BTClibEccValueError("not the same curve for all signatures")
