...
from btclib_ecc.alias import INF, BinaryData, Integer, Octets, Point
...
from btclib_ecc.ecc.borromean import BorromeanSig, PubkeyRing, _hash
...
from btclib_ecc.ecc.rfc6979_nonce import _HmacDrbg
...
def _assert_valid_mantissa(mantissa: int) -> None:
    ...
    if not 1 <= mantissa <= _MAX_MANTISSA:
        raise BTClibEccValueError(f"rangeproof mantissa not in 1..64: {mantissa}")
...
def _assert_valid_header(exp: int, mantissa: int, min_value: int | None) -> None:
    ...
    if mantissa:
        if not 0 <= exp <= _MAX_EXP:
            raise BTClibEccValueError(f"rangeproof exponent not in 0..18: {exp}")
    elif exp != -1:
    ...
    for i, size in enumerate(rsizes):
        ...
        for j in range(size):
            ...
            if not 0 < s < secp256k1.n:
                raise BTClibEccRuntimeError("rangeproof nonce is not a scalar")
            ring.append(s)
...
class RangeProof:
    ...
    def parse(
        ...
        if flags & _RESERVED:
            raise BTClibEccValueError("bit 7 set in the rangeproof flags")
        exp, mantissa = -1, 0
        ...
        if padding and octets[-1] >> padding:
            raise BTClibEccValueError("rangeproof sign bit padding is not zero")
...
def _challenge(m: bytes, r: bytes, i: int, j: int) -> int:
    ...
    if not 0 < e < secp256k1.n:
        raise BTClibEccRuntimeError("rangeproof challenge is not a scalar")
    return e
...
def _borromean_sign(
    ...
    for i, (ring, k, j_star) in enumerate(
        ...
        if s[i][j_star] == 0:
            ...
            raise BTClibEccRuntimeError("rangeproof signature value is zero")
...
def sign(
    ...
    *,
    ...
    if not 0 <= min_bits <= _MAX_MANTISSA:
        raise BTClibEccValueError(f"rangeproof min bits not in 0..64: {min_bits}")
    ...
    if secs[-1] == 0:
        raise BTClibEccRuntimeError("rangeproof last ring blinding factor is zero")
...
def _borromean_verify(
    ...
    if sha256(e0_preimage + m).digest() != sig.e0:
        raise BTClibEccRuntimeError("rangeproof signature verification failed")
...
def verify(
    ...
    *,
    ...
    `BTClibEccValueError` from `assert_as_valid`. See `_proof_from`.
    """
    parsed_proof = _assert_structurally_valid_(proof)
    # ValueError and BTClibEccRuntimeError: a well-formed proof that does
...
def rewind(
    ...
    *,
    ...
    if rsizes == (1,):
        ...
        if found is None:
            raise BTClibEccRuntimeError("rangeproof rewind reads no value encoding")
        skip1, v = found
        skip2 = (v >> 2 * last) & 3
        if skip1 == skip2:
    ...
    if _commit(blind, value, gen, secp256k1) != commitment:
        raise BTClibEccRuntimeError("rangeproof rewind does not open the commitment")
