...
from btclib_ecc.curves.curve import _add, _assert_valid_ec
...
def commit(r: Integer, v: Integer, gen: Point, ec: Curve = secp256k1) -> Point:
    ...
    already turns the `BTClibEccValueError` this raises into `False`, the
...
def verify(
    ...
    # ValueError and BTClibEccRuntimeError: a well-formed opening that does
...
def _point_from_octets(octets: Octets, tag: int, name: str) -> Point:
    ...
    if prefix & 0xFE != tag ^ 1:
        raise BTClibEccValueError(f"not a {name}: prefix 0x{prefix:02x}")
