...
the curve or the hash function, as in `btclib_ecc.ecc.musig2`: BIP374 is
...
`btclib_ecc.ecc.ssa`. It binds the proof to a statement of the protocol
...
def generate_proof(
    ...
    A BTClibEccValueError for an `a` outside 1..n-1 or a B, G that is no
    ...
    if not verify_proof(
        ...
        raise BTClibEccRuntimeError("implausible proof failure")
    return proof
...
def assert_proof_as_valid(
    ...
    if R1[1] == 0:
        raise BTClibEccValueError("invalid (INF) R1")
    R2 = double_mult_var(s, B_point, -e, C_point)
    if R2[1] == 0:
        raise BTClibEccValueError("invalid (INF) R2")

    if e != _challenge(A_point, B_point, C_point, R1, R2, G_point, m):
        raise BTClibEccValueError("invalid challenge")
