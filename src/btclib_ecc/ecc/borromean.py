...
from btclib_ecc.curves.curve import _assert_valid_ec
...
def _bytes_from_ring_point(Q: Point, ec: Curve, ring: int, position: int) -> bytes:
    ...
    # BTClibEccValueError with neither
    try:
        return bytes_from_point(Q, ec)
    except BTClibEccValueError as exc:
...
class BorromeanSig:
    ...
    def assert_valid(self) -> None:
        ...
        if not self.s:
            raise BTClibEccValueError("no rings")
...
def sign_(
    ...
    # not do quietly. zip's own message is a BTClibEccValueError's class
...
def sign(
    ...
    `pubk_rings[i]`, refused with `BTClibEccValueError` naming the ring,
...
def verify(
    ...
    # ValueError and BTClibEccRuntimeError: a well-formed signature whose
...
def assert_as_valid(
    ...
    refused first, as a plain `BTClibEccValueError` naming the ring and the
