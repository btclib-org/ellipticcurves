...
is what turns the shared secret of :mod:`btclib_ecc.ecc.dh` into an actual
...
:mod:`btclib_ecc.ecc`: every implementation that exists is secp256k1 with
...
from btclib_ecc._utils import assert_type, bytes_from_octets, str_from_string
from btclib_ecc.alias import CipherF, Integer, Octets, String
...
from btclib_ecc.curves.sec_point import _mult_sec, _sec_from_pub_key
...
def derive_keys(prv_key: Integer, pub_key: PubKey) -> tuple[bytes, bytes, bytes]:
    ...
    :mod:`btclib_ecc.ecc.dh`'s verdict for every ECDH-shaped computation
    ...
    :func:`btclib_ecc.ecc.dh.diffie_hellman`, which takes a bare int: the
...
class Envelope:
    ...
    def assert_valid(self) -> None:
        ...
        if len(self.magic) != _MAGIC_SIZE:
            raise BTClibEccValueError(f"invalid magic size: {len(self.magic)} bytes")
        ...
        if len(self.mac) != _MAC_SIZE:
            raise BTClibEccValueError(f"invalid MAC size: {len(self.mac)} bytes")
    ...
    def b64decode(
        ...
        try:
            ...
            raise BTClibEccValueError(f"invalid base64 encoding: {e}") from e
        if base64.b64encode(data_decoded) != data_bin:
            raise BTClibEccValueError("invalid base64 encoding: not canonical")
...
def decrypt(
    ...
    *,
    ...
    BTClibEccRuntimeError and the caller's cipher never runs.
    """
