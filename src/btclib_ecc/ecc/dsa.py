...
sign-to-contract style (see btclib_ecc.ecc.commit_nonce for the tweak), and
...
from btclib_ecc.alias import BinaryData, HashF, Integer, JacPoint, Octets, Point
...
from btclib_ecc.curves.sec_point import _sec_from_pub_key
from btclib_ecc.ecc.commit_nonce import (
    commit_entropy_,
    commit_nonce_,
    commit_point_,
)
from btclib_ecc.ecc.rfc6979_nonce import _rfc6979_nonce_, challenge_
from btclib_ecc.exceptions import (
    BTClibEccRuntimeError,
    BTClibEccTypeError,
    BTClibEccValueError,
)
from btclib_ecc.hashes import _assert_valid_hf, reduce_to_hlen
...
def _parse_der_value(stream: BytesIO) -> bytes:
    ...
    if len(value) != size:
        raise BTClibEccValueError("invalid DER length: not enough binary data")
    return value
...
def _deserialize_scalar(sig_data_stream: BytesIO, strict: bool) -> int:
    ...
    if strict:
        ...
        if len(scalar_bytes) > 1 and scalar_bytes[0] == 0 and scalar_bytes[1] < 0x80:
            raise BTClibEccValueError("invalid 'highest bit set' padding")
        if scalar_bytes[0] >= 0x80:
            raise BTClibEccValueError("invalid negative scalar")
    ...
    def parse(
        ...
        *after* the sequence as well as what is in it: a byte too many is
        ...
        if strict and stream.read(1) != b"":
            raise BTClibEccValueError("trailing bytes after the DER sequence")
...
def _sign_recoverable_(
    ...
    if r == 0:  # r≠0 required as it multiplies the public key
        raise BTClibEccRuntimeError("failed to sign: r = 0")
    ...
    if s == 0:  # s≠0 required as verify will need the inverse of s
        raise BTClibEccRuntimeError("failed to sign: s = 0")
...
def _python_verified(
    ...
    except (ValueError, BTClibEccRuntimeError):
        return False
    return True
...
def _abort_unless_checked(
    ...
    Raises:
        BTClibEccValueError: if the signature verifies under the private
            key's own public key and not under the one given.
        BTClibEccRuntimeError: if it verifies under neither, which is the
    ...
    if given is not None:
        ...
        if verified(derive()):
            raise BTClibEccValueError("the public key given is not this private key's")
        raise BTClibEccRuntimeError("signing produced a signature that does not verify")
...
def _libsecp256k1_sign_(
    ...
    *,
    ...
    # holds the foreign call and nothing else: a `BTClibEccValueError` this
    ...
    try:
        ...
        # package's: a caller catching BTClibEccValueError should not have
        ...
        raise BTClibEccValueError(str(e)) from e
    except (
        RuntimeError
        ...
        raise BTClibEccRuntimeError(str(e)) from e
...
def sign_(
    ...
    *,
    ...
    btclib_ecc.ecc.commit_nonce). Keyword-only, and the only argument that
    ...
    if pub_key is not None and not verify:
        raise BTClibEccValueError("pub_key is for the check that verify=False declines")
    ...
    if commit_hash is not None and nonce is not None:
        raise BTClibEccValueError("a commitment derives its own nonce")
    ...
    if grind and (nonce is not None or commit_hash is not None):
        raise BTClibEccValueError("grinding derives its own nonce")
...
def _assert_as_valid_(
    ...
    *,
    ...
    if lower_s and s > ec.n // 2:
        raise BTClibEccValueError("not a low s")
...
def _assert_commitment_(
    ...
    # the two come together or not at all, and a BTClibEccTypeError says
    ...
    if commit_hash is None:
        if receipt is not None:
            raise BTClibEccTypeError("receipt without the commitment it opens")
    elif receipt is None:
        raise BTClibEccTypeError("commitment without the receipt that opens it")
    else:
...
def assert_as_valid_(
    ...
    *,
    ...
    # the (ValueError, BTClibEccRuntimeError) `verify_` catches, and
...
def _assert_structurally_valid_(
    ...
    *encoding* is malformed -- the file's `InvalidEncoding`,
    ...
    this library owes a `BTClibEccTypeError`.
...
def verify_(
    ...
    *,
    ...
    # ValueError and BTClibEccRuntimeError, not Exception: a well-formed
    ...
    #
    # BTClibEccRuntimeError by name and not RuntimeError, because
...
def _recover_pub_keys_(
    ...
    for key_id in range(2 * (ec.cofactor + 1)):
        ...
        with contextlib.suppress(BTClibEccValueError, BTClibEccRuntimeError):
...
def recover_pub_keys_(
    ...
    if _libsecp256k1_serves(sig.ec, hf):
        ...
        for key_id in range(4 if sig.r + sig.ec.n < sig.ec.p else 2):
            ...
            # arrives as the BTClibEccValueError _libsecp256k1_recover_sec_
            # maps it to
            with contextlib.suppress(BTClibEccValueError, BTClibEccRuntimeError):
                keys.append(
...
def _libsecp256k1_recover_sec_(
    ...
    try:
        ...
        # that recovers nothing -- BTClibEccValueError when x_K misses the
        # curve, BTClibEccRuntimeError when the key it yields does not
        ...
        # a subclass
        raise BTClibEccValueError(f"invalid key_id or signature: {e}") from e
...
def _recovery_inputs(
    ...
    if not is_integer(key_id):
        raise BTClibEccTypeError(f"non-integer key_id: {key_id}")
...
def crack_prv_key_var_(
    ...
    ec = sig2.ec
    if sig1.ec != ec:
        raise BTClibEccValueError("not the same curve in signatures")
    if sig1.r != sig2.r:
        raise BTClibEccValueError("not the same r in signatures")
    if sig1.s == sig2.s:
        raise BTClibEccValueError("identical signatures")
