...
from btclib_ecc._libsecp256k1 import keys as libsecp256k1_keys
...
from btclib_ecc.curves.curve import _libsecp256k1_serves, _tweak_add_var
...
def commit_nonce_(
    ...
    if _libsecp256k1_serves(ec, None):
        try:
            ...
            raise BTClibEccRuntimeError("failed to sign: zero tweaked nonce") from e
    ...
    if tweaked_nonce == 0:
        raise BTClibEccRuntimeError("failed to sign: zero tweaked nonce")
