...
signature that verifies under Q with `btclib_ecc.ecc.ssa` and with any other
...
`btclib_ecc.ecc`. BIP327 is defined for that pair alone: the tags below, the
...
The message is of any size, as in `btclib_ecc.ecc.ssa`: BIP327 states no
...
from btclib_ecc._libsecp256k1 import musig as libsecp256k1_musig
from btclib_ecc._utils import assert_type, bytes_from_octets, is_octets
from btclib_ecc.alias import INF, Integer, Octets, Point
from btclib_ecc.curves import mult, multi_mult_var, scalar_from_prv_key, secp256k1
from btclib_ecc.curves.curve import _libsecp256k1_serves, _sum_var, _tweak_add_var
...
def key_agg(pub_keys: Sequence[Octets]) -> KeyAggContext:
    ...
    if (
        ...
        raise BTClibEccRuntimeError("aggregate key is the infinity point")
...
def apply_tweak(
    ...
    if Q[1] == 0:
        ...
        raise BTClibEccValueError(_TWEAK_INF_ERR)
...
def session_values(session_ctx: SessionContext) -> SessionValues:
    ...
    except BTClibEccValueError as err:
...
def _session_key_agg_coeff(session_ctx: SessionContext, pub_key: bytes) -> int:
    ...
    if pub_key not in values.pub_keys_set:
        raise BTClibEccValueError(_SIGNER_PK_ERR)
...
def sign(sec_nonce: bytearray, prv_key: Integer, session_ctx: SessionContext) -> bytes:
    ...
    if pk != bytes(sec_nonce[2 * _SCALAR_SIZE :]):
        raise BTClibEccValueError("Public key does not match nonce_gen argument")
...
def partial_sig_verify_(
    ...
    the same other way, `_SIGNER_PK_ERR`, so which `BTClibEccValueError` a
    ...
    if (
        ...
        try:
            ...
            # not a point is a BTClibEccValueError
...
class PreSignature:
    ...
    (`btclib_ecc.ecc.musig2`'s own docstring says so), so there is no wire
...
def partial_sig_agg(psigs: Sequence[Octets], session_ctx: SessionContext) -> ssa.Sig:
    ...
    the x-only aggregate key with `btclib_ecc.ecc.ssa.verify_` and with
