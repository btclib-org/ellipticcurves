...
def _assert_valid_keying_data_size(size: int, max_size: int) -> None:
    ...
    if not is_integer(size):
        raise BTClibEccTypeError(f"non-integer keying data size: {size}")
    ...
    if size <= 0:
        raise BTClibEccValueError(f"invalid keying data size: {size}")
    if size > max_size:
        raise BTClibEccValueError(f"cannot derive a key larger than {max_size} bytes")
...
def ansi_x9_63_kdf(z: bytes, size: int, hf: HashF, shared_info: bytes | None) -> bytes:
    ...
    BTClibEccTypeError if it is no integer, a BTClibEccValueError if
...
def hkdf_expand(prk: bytes, size: int, hf: HashF, info: bytes | None) -> bytes:
    ...
    BTClibEccTypeError if it is no integer, a BTClibEccValueError if
    it is zero, negative, or above 255 digests. A `prk` below a digest is a
    ...
    if len(prk) < hf_size:
        raise BTClibEccValueError(f"pseudorandom key shorter than {hf_size} bytes")
