...
These exist only to tell an exception raised by btclib_ecc from one
raised by any other code: each derives from the built-in that says what
kind of failure it is, and adds nothing to it.

`BTClibEccException` is what makes that telling apart a single
...
it, which is the half that matters: `BTClibEccValueError` is a
...
class; `except BTClibEccException` is for the caller who only needs
...
class BTClibEccException(Exception):  # noqa: N818 -- a kind, like Exception itself, not a leaf raised
    """Anything btclib_ecc raised, whatever kind of failure it is.
...
class BTClibEccValueError(BTClibEccException, ValueError):
    """A value no valid input could carry; the package's usual refusal."""


class BTClibEccTypeError(BTClibEccException, TypeError):
    """An input of a type no conversion accepts: a caller error."""


class BTClibEccRuntimeError(BTClibEccException, RuntimeError):
    """A check that failed on valid inputs, e.g. a failed verification."""


class InvalidContributionError(BTClibEccRuntimeError):
    ...
    A BTClibEccRuntimeError and not a BTClibEccValueError, which is
...
class BorromeanRingError(BTClibEccRuntimeError):
    ...
    `btclib_ecc.ecc.borromean.sign` and `assert_as_valid` already have both
    ...
    A BTClibEccRuntimeError and not `InvalidContributionError`: that class
    ...
    A BTClibEccRuntimeError still, so code catching that keeps catching
