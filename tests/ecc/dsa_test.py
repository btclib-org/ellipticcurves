...
# signature; `msg_hash` is this package's own segwit v0 `sig_hash.from_tx` for
# the transaction Core built, which is what the signature verifying under it
# confirms.
#
...
def test_libsecp256k1_recovery(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    for q in (0x1, 0x2, prv_key_int, secp256k1.n - 1):
        ...
        # and that is now the whole of the rule
...
def test_the_recovered_key_can_be_infinity_and_is_refused(
    ...
    that replaced it is not a shortcut's price but its gain (issue
    btclib-org/btclib#890): with Q at infinity the recomputed K' is the lift
    itself, so the congruence x_K % ec.n == r passes and the enumeration
    reported the INF as a recovered key -- (5, 0) once converted, which is this
    package's sentinel for infinity and no public key at all.
    `ssa._recover_pub_key_` has always refused its own, for this same reason.
...
def test_recover_pub_key_dispatches_to_the_bindings_for_0_to_3_only() -> None:
    ...
    to instead. Four key_ids, one per boundary the six ways this guard survived
    moved.
    """
...
def test_the_wrong_key_is_told_apart_from_a_wrong_computation(
    ...
    mistyped. `EllipticCurvesRuntimeError` stays what it has always meant here.
    """
    if not bindings:
        no_bindings(monkeypatch)
