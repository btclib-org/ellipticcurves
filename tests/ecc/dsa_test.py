...
from ellipticcurves.alias import INF, JacPoint, Point
...
from ellipticcurves.curves.curve_group import CurveGroup, _mult
from ellipticcurves.ecc import dsa
from ellipticcurves.ecc.rfc6979_nonce import challenge_
...
pub_key_point = _KEYS.Q
pub_key_compressed = _KEYS.Q_compressed
prv_key_int = _KEYS.q
prv_key_hexstring = _KEYS.q_hexstring
wif_compressed_string = _KEYS.wif_compressed_string
wif_uncompressed_string = _KEYS.wif_uncompressed_string
xprv_string = _KEYS.xprv_string


def test_signature_on_an_equal_curve() -> None:
    """A curve equal to secp256k1 is secp256k1, bindings included."""
    # guards against the dispatch comparing identities, which sends any other
    # object holding the secp256k1 parameters down the Python path in silence:
    # the answer must be the one the singleton gives, and RFC6979 makes it
    # deterministic, hence comparable (issue btclib-org/btclib#142)
...
def test_parse_stops_at_the_end_of_the_sequence() -> None:
    ...
    Guards against it being dropped: `Sig.parse(der + b"\\x01")` answering with
    the `Sig` of `der` is how a two-byte hash type reaches verification as a
    valid signature (issue btclib-org/btclib#129). Core rejects the element with
    one size equation, `(lenR + lenS + 7) != sig.size()` in
    IsValidSignatureEncoding, and only under the flags asking for canonical DER:
    hence strict here, and hence the lenient parse the engine uses when DERSIG
    is off still takes it.
    """  # noqa: D301
...
def test_signature() -> None:
    ...
    # signature has standing to refuse it (issue btclib-org/btclib#695)
    ...
    with pytest.raises(EllipticCurvesValueError, match="scalar r not in 1..n-1: "):
        dsa.Sig(sig.ec.n, sig.s)
    with pytest.raises(EllipticCurvesValueError, match="scalar s not in 1..n-1: "):
...
def _step_1_6_1(c: int, r: int, s: int, ec: Curve) -> list[Point | None]:
    ...
    The step written out with the curve's affine arithmetic, as the independent
    opinion `_recover_pub_key_` is held against: a candidate is r^-1*(s*K - c*G)
    over the lift of x_K = r + j*ec.n, and None where that x_K is no field
    element, or no x-coordinate of the curve, or where the sum is INF. On a
    curve of prime order those three are the whole of what step 1.6 refuses,
    which is what issue btclib-org/btclib#890 turns on, so this list less its
    Nones is what the enumeration has to answer.
    """
        ...
        for k in range(1, ec.n):  # all possible ephemeral keys
            ...
            for e in range(ec.n):  # all possible challenges
                ...
                if r == 0 or s == 0:
                    ...
                    # every key it answers is a key, and the INF a list
                    # could carry is none: with Q at infinity the
...
def test_key_id_is_the_j_zero_pair_when_n_is_above_p() -> None:
    """The mirror of the case above, and btclib-org/btclib#183's n > p box.
...
def test_crack_prv_key() -> None:
    ...
    with pytest.raises(EllipticCurvesValueError, match="not the same r in signatures"):
        dsa.crack_prv_key_var(msg1, sig1, msg2, dsa.Sig(16, sig1.s))

    with pytest.raises(EllipticCurvesValueError, match="identical signatures"):
    ...
    with pytest.raises(
        EllipticCurvesValueError, match="not the same curve in signatures"
    ):
...
def test_prv_key_is_not_a_pub_key() -> None:
    ...
    # carrying a private key and refused for what they hold. No `type: ignore`
    # on these: they type check, and none of the four is a spelling
    # `point_from_pub_key` accepts, which is a structurally invalid key and
    # raises (issue btclib-org/btclib#2170) rather than reading as a signature
    ...
    for prv_key in (
        ...
        with pytest.raises(EllipticCurvesValueError) as refusal:
            dsa.assert_as_valid(msg, prv_key, sig)
        assert str(prv_key) not in str(refusal.value)

    # and the int, which a PubKey does not declare at all: refused as a type
    # rather than answered False, which is issue btclib-org/btclib#814's rule
    # and the stronger half of this issue's. Somebody passing an int here meant
    # a private key, where False reads as a signature that did not verify.
    # `verify` refuses it as `assert_as_valid` does, a TypeError being outside
    # the `except` that turns a refusal into False -- and the `type: ignore`
    # these two need, where the five above need none, is the line this rule
    # draws, written out by the type checker
    for call in (dsa.verify, dsa.assert_as_valid):
        with pytest.raises(
            EllipticCurvesTypeError, match="not a public key"
        ) as wrong_type:
    ...
    # An xpub is not among them: a caller holding one passes the point it
    # carries (issue btclib-org/btclib#1188)
    for pub_key in (
        pub_key_point,
        pub_key_compressed,
...
def test_libsecp256k1() -> None:
    """Verify this package and the bindings sign and verify byte-for-byte."""
    msg = b"Satoshi Nakamoto"
...
def test_a_plain_sign_reaches_the_bindings(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    lower_s and commit_hash is None` -- is every default a caller who passes
    none of the three keeps, and both arms it chooses between reach the same
    signature from the same secret key: nothing about the result says which one
    produced it (issue btclib-org/btclib#975, asked in general of every guard a
    computed value could disable rather than of this one, which has none). What
    differs is not the nonce's own point -- `mult(k, ec.G, ec)` reaches
    libsecp256k1 either way while it serves, SECURITY.md's own accounting of it
    -- but its modular inverse, which this call does inside libsecp256k1's own
    constant time and the Python arm below draws as a blinded Python integer
    instead. This records the call into `libsecp256k1_dsa.sign` instead of the
    answer it returns, so a change that silently stopped taking it would fail
    here rather than reproduce the same signature from the arm it stands in for.
    """
    calls: list[int] = []
...
# Core's low-R grinding (issue btclib-org/btclib#638): the loop is
# `dsa._grind_low_r`, and what follows holds it to the three implementations
# that have it -- Core's `CKey::Sign` since its PR 13666, electrum-ecc's
# `ECPrivkey.ecdsa_sign` and embit's `PrivateKey.sign`. 2**255 is the bound all
# three grind towards: r at or above it has its highest bit set, and DER then
# pays a 0x00 byte to keep it from reading as negative
_LOW_R = 2**255
...
def test_grinding_is_about_r_and_not_about_the_encoded_length() -> None:
    ...
    always, and is one this package reaches on purpose. `lower_s=False` is also
...
# `createrawtransaction` spends outpoint 0101..01:0 -- a p2wpkh output of the
# key, worth 1 BTC -- into a p2wpkh output of the same key worth the value named
# below, and `signrawtransactionwithkey` signs that with the key's WIF and that
# prevout. Core grinds by default, so the witness item it produces is its ground
# signature; `msg_hash` is btclib-org/btclib's segwit v0 `sig_hash.from_tx`
# for the transaction Core built, which is what the signature verifying under
# it confirms.
#
...
def test_a_sig_that_never_validated_answers_false_and_does_not_raise() -> None:
    """Why `assert_as_valid_` validates the `Sig` given (btclib-org/btclib#888).
    ...
    Both of those reach `_serialize_scalar`, where `to_bytes(..., signed=False)`
    raises OverflowError for a negative r -- an ArithmeticError, so not in the
    `(ValueError, EllipticCurvesRuntimeError)` tuple `verify_` catches. Drop the
    validation and a function whose whole answer is True or False raises
    instead, which is the rule issue btclib-org/btclib#814 states.
    """
    _, Q = dsa.gen_keys(prv_key_int)
...
def test_libsecp256k1_recovery(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    for q in (0x1, 0x2, prv_key_int, secp256k1.n - 1):
        ...
        # negating s mirrors the nonce's point, so the candidate that recovers
        # the signer is the one whose parity bit flipped -- and the malleated
        # signature does recover it, on both implementations, no lower-s rule
        # refusing what the signer was free to produce (issue
        # btclib-org/btclib#695). The recoverable parser takes any s in [1, n-1]
...
def test_the_low_s_rule_is_asked_for_and_not_assumed(
    ...
    """Where the strict rule lives (btclib-org/btclib#695).

    The rule is policy: `SCRIPT_VERIFY_LOW_S` is in Core's
    ...
    for that reason. A verifier here does not refuse one -- so the
...
def test_recover_pub_keys_takes_the_hash_that_recover_pub_keys_reduces() -> None:
    ...
    Nothing in this package calls either, so this is what holds the
    pairing that justifies both.
    ...
    form s took having been the signer's choice (issue btclib-org/btclib#695).
    """
    msg = b"Satoshi Nakamoto"
    for q in (0x1, 0x2, prv_key_int, secp256k1.n - 1):
        prv_key, Q = dsa.gen_keys(q)
...
def test_recovery_multiplies_in_libsecp256k1(
    ...
    """The enumeration's three implementations agree (btclib-org/btclib#286).

    On secp256k1 with sha256 `recover_pub_keys` is four
    `secp256k1_ecdsa_recover` calls: the enumeration is this package's loop --
...
def test_the_enumeration_asks_for_the_j_one_candidates(
    ...
    agree about there (issue btclib-org/btclib#891).
    """
    msg_hash = reduce_to_hlen(b"Satoshi Nakamoto")
    sig = dsa.Sig(r, 12345)
...
def test_the_recovered_key_can_be_infinity_and_is_refused(
    ...
    Step 1.6.2's verification does not catch it, which is why the infinity test
    standing in its place is not a shortcut's price but its gain (issue
    btclib-org/btclib#890): with Q at infinity the recomputed K' is the lift
    itself, so the congruence x_K % ec.n == r passes and without the test the
    enumeration would report the INF as a recovered key -- (5, 0) once
    converted, which is this package's sentinel for infinity and no public key
    at all. `ssa._recover_pub_key_` refuses its own, for this same reason.
...
def test_sign_recoverable_is_sign_plus_the_key_id_a_search_would_find(
    ...
    signing time, which is the whole of issue btclib-org/btclib#285.
...
def test_recover_pub_key_dispatches_to_the_bindings_for_0_to_3_only() -> None:
    ...
    A key_id outside that range has to reach the Python path instead, which
    answers in its own words rather than the bindings': its own screen on x_K =
    r + j*ec.n, which is no field element for a j of -1 or 2 (issue
    btclib-org/btclib#891) -- not the bindings' "the recovery id must be 0, 1,
    2, or 3", which is what a widened or narrowed guard sends a boundary value
    to instead. Four key_ids, one per boundary a mutant of this guard can
    move.
    """
    ...
    cases = {
        -1: (EllipticCurvesValueError, r"invalid key_id \(-1\)"),
        2: (EllipticCurvesValueError, "public key recovery failed"),
        3: (EllipticCurvesValueError, "public key recovery failed"),
        4: (EllipticCurvesValueError, r"invalid key_id \(4\)"),
    }
            ...
            for k in range(1, ec.n):
                for c in range(ec.n):
                    for lower_s in (True, False):
                        ...
                        except EllipticCurvesRuntimeError:  # r == 0 or s == 0
                            continue
                        QJ = dsa._recover_pub_key_(
...
def test_libsecp256k1_py_vectors_ecdsa(vector: dict[str, str]) -> None:
    ...
    pub_key = bytes_from_point(mult(scalar_from_prv_key(prv_key)))
...
def test_verify_with_another_hash_function_on_both_arithmetics(
    ...
    def checks() -> None:
        ...
        with pytest.raises(EllipticCurvesRuntimeError, match=r"invalid \(INF\) key"):
            dsa._assert_as_valid_(
                ec.n - 1, ec.GJ, 1, 1, ec, ec._fixed_points, lower_s=False
            )
...
def test_verify_answers_about_signatures_not_about_types() -> None:
    ...
    # `wycheproof_test.py::test_ecdsa_der` is measured against several thousand
    # vectors built on exactly that question (issue btclib-org/btclib#2170's
    ...
    # EllipticCurvesRuntimeError is caught by name and not as RuntimeError, so a
    # RecursionError -- which is a RuntimeError -- is not swallowed either
    assert issubclass(RecursionError, RuntimeError)
    assert not issubclass(RecursionError, EllipticCurvesRuntimeError)
...
def test_a_bad_hf_raises_rather_than_answering_about_the_signature() -> None:
    ...
    (ValueError, EllipticCurvesRuntimeError)` reports anything it catches as a
    signature that does not verify, so the refusal has to be the
    EllipticCurvesTypeError `hashes._assert_valid_hf` raises and has to
    happen before the try. `sha256()` for `sha256` is the mistake it is written
    for (issue btclib-org/btclib#745).
    """
    msg = b"Satoshi Nakamoto"
    q, Q = dsa.gen_keys(0x1)
    sig = dsa.sign(msg, q)
...
def test_a_prepared_key_stops_the_per_signature_table(
    ...
    This is issue btclib-org/btclib#893 itself, and it is asserted as a count
    rather than as a duration: what a prepared key buys is that the wNAF tables
    of the key's two GLV halves are memoized instead of built and dropped, and
    the tables built per call is the thing that changes. A time would measure
    the machine.
...
def test_a_key_that_is_no_point_is_refused_by_the_verification_itself(
    ...
    The delegated path does not prove the key before verifying with it: the
    verification's own parse is that proof, and proving it here as well would
    lift one x twice (issue btclib-org/btclib#887). So what a bad key raises
    comes from the bindings and is translated, where the Python path raises it
    while converting -- the two messages are held equal here, that being the
    only thing a caller sees of the difference.
    """
    if not bindings:
        no_bindings(monkeypatch)
...
def test_the_wrong_key_is_told_apart_from_a_wrong_computation(
    ...
    mistyped. `EllipticCurvesRuntimeError` means only the fault here.
    """
    if not bindings:
        no_bindings(monkeypatch)
    ...
    with pytest.raises(EllipticCurvesValueError, match="not this private key's"):
...
def test_a_key_is_refused_beside_the_flag_that_declines_the_check(
    ...
    q, Q = dsa.gen_keys(0x1234567890ABCDEF)
    with pytest.raises(EllipticCurvesValueError, match="verify=False declines"):
...
def test_the_two_arms_answer_the_same_refusals(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    def answers() -> tuple[Any, ...]:
        ...
        with pytest.raises(EllipticCurvesValueError) as wrong_key:
            dsa.sign(msg, q, pub_key=bytes_from_point(other))
        with pytest.raises(EllipticCurvesValueError) as contradiction:
        ...
        with pytest.raises(EllipticCurvesValueError) as no_point:
...
def test_a_key_that_is_no_point_is_refused_before_anything_is_signed(
    ...
    both arms here is that the refusal arrives and is this package's own
    exception.
...
def test_a_fault_is_not_reported_as_a_wrong_key() -> None:
    ...
    `EllipticCurvesRuntimeError` is reachable from an argument -- what they
    ...
    with pytest.raises(EllipticCurvesRuntimeError, match="does not verify"):
...
def test_the_delegated_grind_is_the_sequence_the_py_arm_walks() -> None:
    ...
    already do and pay this package's own crossing per attempt -- two on
    average.
    ...
    for i in range(1, 61):
        ...
        # and it is the signature this package's own loop reaches, which is the
...
def test_the_delegated_arm_asks_the_bindings_to_verify(
    ...
    Nothing before this recorded it: the crossing above shows the two arms agree
    on the signature `grind=True, verify=False` produces, not that `verify` is
    the argument deciding whether a check follows it, or that grinding pays for
    one crossing and not one per attempt (issue btclib-org/btclib#986).
    `_grind_low_r`'s own loop is the Python arm's; the delegated arm's grinding
    is the bindings' `dsa._grind`, which runs below the `sign` this patches and
    so is invisible here -- it reaches `secp256k1_ecdsa_sign` once per attempt,
    where what this counts is this package crossing into `sign` once per `sign_`
    call, whether that call grinds or not.
    """
    real_sign = libsecp256k1_dsa.sign
    calls: list[bool] = []
    ...
    with monkeypatch.context() as patch:
        ...
        for grind in (True, False):
            for verify in (True, False):
                ...
                # one crossing, carrying the caller's own verify -- not zero,
                # which is btclib-org/btclib#982's bill, and not one per
                # attempt, which is the grinding loop this arm does not walk
                assert calls == [verify]
...
def test_sign_recoverable_asks_the_bindings_to_verify(
    ...
    `sign_recoverable_` takes no `verify` of its own: the comment at its call
    into the bindings argues why True belongs there rather than at the bindings'
    default, the recovery id being a value nothing downstream re-derives the way
    a faulted r or s is caught by the first verification anybody makes of a
    plain signature. Nothing held that argument to the keyword actually written
    before this (issue btclib-org/btclib#986): a refactor dropping it, or
    writing False, changes no signature and no recovered key, and would still be
    caught by nothing.
    """
...
def test_a_delegated_signer_s_wipe_zeroes_the_buffer_it_signs_from() -> None:
    """`wipe` reaches this arm's key, not only the reference to it.
    ...
    signature it has made was read from, and overwriting it reaches
...
@needs_bindings
def test_a_refusal_that_is_not_about_the_key_still_leaves_as_this_package_s(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    ...
    what the branch is for all the same: the hierarchy is this package's, and a
    caller catching `EllipticCurvesValueError` must not have a bare `ValueError`
    ...
    q, _Q = dsa.gen_keys(0x1234)
    with pytest.raises(EllipticCurvesValueError, match="a refusal of some other kind"):
        dsa.sign(b"Satoshi Nakamoto", q, pub_key=None)


def test_verify_raises_on_a_structurally_invalid_public_key() -> None:
    """A public key no conversion accepts raises (btclib-org/btclib#2170).
...
def test_verify_raises_on_a_digest_that_is_not_hf_s_size() -> None:
    ...
    message it declares has no size, so a short one is a message that was not
    signed, and answering False about it is issue btclib-org/btclib#814's rule
    untouched.
    """
    q, Q = dsa.gen_keys(0x1234567890ABCDEF)
    msg = b"Satoshi Nakamoto"
...
def test_verify_answers_false_for_a_malformed_der_encoding() -> None:
    ...
    Unlike a public key or BIP340's own fixed 64 bytes, a DER signature's own
    grammar is what `wycheproof_test.py::test_ecdsa_der` measures against
    vectors built on exactly this question, over the bitcoin, generic ecdsa,
    sha512, sha3 and shake profiles: a signature whose encoding is malformed
    answers False. Reclassifying it as structural here would fail that suite, so
    it stays inside the try, which is the one case issue btclib-org/btclib#2170
    found that does not mechanically follow BIP340's model and is left as it
    was.
    """
    ...
    # each spelling with the message argument it declares: `verify` takes one of
    # any size, `verify_` a digest of hf's, which since issue
    # btclib-org/btclib#2170 it refuses at any other length -- so the raw
    # message cannot stand in for the digest here without answering a different
    # question
...
@pytest.mark.parametrize(
    ...
    [
        pytest.param(
            b"",
            ("EllipticCurvesValueError", "not enough binary data for var_int"),
            id="nothing",
        ),
        pytest.param(
            b"\x00",
            ("EllipticCurvesValueError", "invalid DER length: zero size"),
            id="zero size",
        ),
        pytest.param(b"\x02ab", ("value", b"ab"), id="one octet"),
        pytest.param(
            b"\x03ab",
            ("EllipticCurvesValueError", "invalid DER length: not enough binary data"),
            id="overrun",
        ),
        pytest.param(b"\x80" + bytes(0x80), ("value", bytes(0x80)), id="0x80"),
        pytest.param(b"\xfc" + bytes(0xFC), ("value", bytes(0xFC)), id="0xfc"),
        pytest.param(
            b"\xfd\xfd\x00" + bytes(0xFD), ("value", bytes(0xFD)), id="two octets"
        ),
        pytest.param(
            b"\xfd\xfd",
            ("EllipticCurvesValueError", "not enough binary data for var_int"),
        ...
        pytest.param(
            ...
            ("EllipticCurvesValueError", "var_int too big: 02000001, max is 02000000"),
            id="four octets past the cap",
        ),
        pytest.param(
            b"\xff" + bytes(8),
            ("EllipticCurvesValueError", "non-canonical var_int: 0 encoded in 9 bytes"),
...
def test_recover_pub_key_validates_a_sig_it_is_handed() -> None:
    """A `Sig` is asked `assert_valid`, as octets are asked by `parse`."""
    # the key is 1, and key_id 1 is the candidate that recovers it
    msg_hash = sha256(b"msg").digest()
    sig = dsa.sign(b"msg", 1)
    assert dsa.recover_pub_key_(1, msg_hash, sig) == secp256k1.G
    assert dsa.recover_pub_key_(1, msg_hash, sig.serialize()) == secp256k1.G
    invalid = dsa.Sig(0, 1, check_validity=False)
    with pytest.raises(EllipticCurvesValueError, match="r not in 1..n-1"):
        dsa.recover_pub_key_(0, msg_hash, invalid)


@pytest.mark.parametrize("compressed", [True, False])
def test_recover_sec_is_the_recovered_key_as_octets(
    monkeypatch: pytest.MonkeyPatch, compressed: bool
) -> None:
    """`recover_sec_` answers `bytes_from_point` of `recover_pub_key_`.

    On both arms: the bindings write the octets themselves, so the two
    serializations are held to each other and not to one shared helper,
    and a candidate recovering nothing is refused alike on each.
    """
    msg = b"a message signed once and recovered as octets"
    msg_hash = reduce_to_hlen(msg)
    for q in (1, prv_key_int, secp256k1.n - 1):
        sig, key_id = dsa.sign_recoverable(msg, q)
        Q = mult(q)
        expected = bytes_from_point(Q, compressed=compressed)
        assert dsa.recover_sec_(key_id, msg_hash, sig, compressed=compressed) == (
            expected
        )
        assert dsa.recover_sec(key_id, msg, sig, compressed=compressed) == expected
        with monkeypatch.context() as python:
            python.setattr(dsa, "_libsecp256k1_serves", lambda *_: False)
            assert dsa.recover_sec_(key_id, msg_hash, sig, compressed=compressed) == (
                expected
            )
        # key_id 2 and 3 recover nothing on secp256k1, on either arm, the
        # bindings' being reachable only where they are installed
        for arm in (True, False) if INSTALLED else (False,):
            with monkeypatch.context() as served:
                served.setattr(dsa, "_libsecp256k1_serves", lambda *_, a=arm: a)
                for beyond_n in (2, 3):
                    with pytest.raises(EllipticCurvesValueError):
                        dsa.recover_sec_(beyond_n, msg_hash, sig, compressed=compressed)

    # a curve the bindings never serve takes the Python arm alone
    ec = CURVES["secp256r1"]
    sig, key_id = dsa.sign_recoverable(msg, prv_key_int, ec=ec)
    assert dsa.recover_sec(key_id, msg, sig, compressed=compressed) == (
        bytes_from_point(mult(prv_key_int, ec.G, ec), ec, compressed)
    )


def test_recover_sec_validates_its_inputs() -> None:
    """The inputs `recover_pub_key_` refuses, and a non-bool `compressed`."""
    msg_hash = sha256(b"msg").digest()
    sig = dsa.sign(b"msg", 1)
    assert dsa.recover_sec_(1, msg_hash, sig.serialize()) == bytes_from_point(
        secp256k1.G
    )
    with pytest.raises(EllipticCurvesTypeError, match="non-integer key_id"):
        dsa.recover_sec_("1", msg_hash, sig)  # type: ignore[arg-type]
    with pytest.raises(EllipticCurvesValueError, match="r not in 1..n-1"):
        dsa.recover_sec_(0, msg_hash, dsa.Sig(0, 1, check_validity=False))
    with pytest.raises(EllipticCurvesValueError):
        dsa.recover_sec_(1, msg_hash[:31], sig)
    with pytest.raises(EllipticCurvesTypeError, match="invalid compressed type"):
        dsa.recover_sec_(1, msg_hash, sig, compressed=1)  # type: ignore[arg-type]


@needs_bindings
def test_recovery_without_the_bindings_is_python_throughout(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """The recovery gated on availability alone must be Python throughout.

    `no_bindings_anywhere` puts the whole of btclib_secp256k1 out of
    reach, module and already-bound name alike, and switches the dispatch
    off, so an arm that still called through would fail rather than pass
    by measuring the bindings against themselves. What the call answered
    with the bindings in reach is what it has to answer without them.
    """
    msg = b"a message signed once and recovered from twice"
    sig = dsa.sign(msg, 1)
    delegated = dsa.recover_pub_keys(msg, sig)
    assert secp256k1.G in delegated

    no_bindings_anywhere(monkeypatch)

    # what dsa itself would have called, so that a walk reaching nothing
