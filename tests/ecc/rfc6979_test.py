...
# `bitcoin/tests/test_wallet.py`, which is where the bitcointalk thread that
# first published the nonce of the first one ended up (issue
# btclib-org/btclib#199). tests/_data/README.md pins the revision; not vendored
# as a file, upstream having none.
#
# Each pins the nonce *and* the signature, which is the half this package had
...
def test_rfc6979_secp256k1_s_is_normalized() -> None:
    ...
    hands back the low s and so does this package by default.
    """
    normalized = 0
