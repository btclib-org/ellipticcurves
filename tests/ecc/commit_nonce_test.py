...
# this package's receipt, and reproducing it pins the derivation of the
...
# Issue btclib-org/btclib#1679's first oracle: the two fixed vectors above pin
# this package's own derivation against libsecp256k1-zkp's fixture, over one key
# and one message; these two ask the same question the other way, over random
# keys, messages and commitments, in both directions -- a commitment made
# here has to open under zkp's own `verify_commit`, and a zkp opening has
# to open under `dsa.verify_`. Neither fixed vector nor a hand-rolled generator:
# `random.seed(42)` at the top of this file is what the rest of it already draws
# from, and every `ecc/` comparison against the bindings does the same rather
# than reaching for hypothesis, which this suite reserves for parsers and
# codecs.
#
...
#
# `pragma: no cover` on every `@needs_zkp` below, the marker being the reason:
...
def test_dsa_commitment_opens_under_zkp() -> None:
    """A sign-to-contract commitment made here opens under zkp's own check."""
    for _ in range(16):
...
# tweak comes after it -- while nothing this package signs opens under
...
def test_zero_tweaked_nonce_through_the_bindings(
    ...
    raise the one error `commit_nonce_` raises for it.
    """
    ec = secp256k1
    nonce = 1 + random.randrange(ec.n - 1)
...
def test_commit_point_falls_back_at_infinity(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    `secp256k1_ec_pubkey_tweak_add` refuses a result at infinity, where `ec.add`
    returns it -- the one behaviour difference between the two halves issue
    btclib-org/btclib#271 calls out, so the fallback has to be exercised rather
    than trusted: a tweak forced to n - k lands exactly there.
    """
    ec = secp256k1
