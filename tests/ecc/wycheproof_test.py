...
(issue btclib-org/btclib#695); the rule itself lives on in `sign` and in the
...
def _generic_valid() -> frozenset[tuple[str, str]]:
    ...
    Which is how the two verdicts this package no longer shares with the bitcoin
    ...
    verifier no longer applies (issue btclib-org/btclib#695). There are two of
