...
first file and `valid` in the second, and this package answers with the first.

What it does not share is the low-s rule, and the two files say so by naming the
same signature twice: tcId 1 and tcId 388 of the bitcoin file are tcId 5 and
tcId 392 of the generic one -- one key and one signature each time, `invalid`
there and `valid` here. This package answers `valid` for all four, which form s
took having been the signer's choice, so a verifier has no standing to refuse it
(issue btclib-org/btclib#695); the rule itself lives in `sign` and in the
leading-underscore spellings. `_generic_valid` below is how those two are found,
and says why a flag would find only one.
...
that is the width `challenge_` reads: any length from there up is the same test,
a SHAKE's output being a stream whose longer forms have these very bytes as
their prefix. What the adapter is not is a route to signing with SHAKE --
`dsa.sign` derives its nonce through RFC6979, which is HMAC, and HMAC over an
XOF is not defined.
...
# its own edge cases, none of which is this package's
...
def _point_from_spki(der: bytes) -> Point:
    ...
    Wycheproof hands an ECDH public key ASN.1-encoded, where this package's key
    boundary is a point and a curve, so something has to read the
    envelope. It is the test's reader and not this package's, and the split is
...
def test_pinned_xof() -> None:
    ...
    A test of the harness rather than of this package, and it earns its place
...
def _generic_valid() -> frozenset[tuple[str, str]]:
    ...
    Which is how the two verdicts this package does not share with the bitcoin
    profile are found, rather than by naming their numbers: one key, one
    signature, `invalid` under the bitcoin profile and `valid` under the generic
    one is a case whose only defect is the form of s, and that is the rule a
    verifier here does not apply (issue btclib-org/btclib#695). There are two of
    them, and a flag reaches only the first -- tcId 1 is
    `SignatureMalleabilityBitcoin`, while tcId 388, "edge case for signature
    malleability", is flagged `ArithmeticError` along with six cases that are
    invalid for their arithmetic and not for their s.
    """
    return frozenset(
...
def test_ecdsa_p1363(
    ...
    This package has no IEEE P1363 parser -- a bitcoin signature is DER -- so
    ...
    is `valid`, which is the answer `verify_` gives everywhere.
