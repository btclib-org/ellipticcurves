...
# needs no name here -- it is no public key, and this package refuses it as the
...
def test_the_message_is_32_bytes_or_absent() -> None:
    ...
    btclib-org/btclib#2170: BIP374 asserts the length in `dleq_challenge`, which
...
def test_a_proof_is_64_bytes() -> None:
    """Anything else is refused, by both spellings (btclib-org/btclib#2170).
...
def test_verify_proof_tells_a_malformed_argument_from_one_that_fails() -> None:
    """btclib-org/btclib#2170's two sides, over the four points and the proof.
