...
def test_challenge_hash_matches_zkps_preimage_order() -> None:
    ...
    secp256k1-zkp's rangeproof module builds this challenge as `e || m || ring
    || pos` (`secp256k1_borromean_hash`,
    `src/modules/rangeproof/borromean_impl.h`, read at its current tip on
    2026-08-20): the point -- or the `e0` closing the rings -- first, then the
    message hash, then the ring index and the position each as 4 bytes
    big-endian (issue btclib-org/btclib#1070). This pins that order computed by
    hand from zkp's documented source, and runs in every build, where
    `test_zkp_verifies_this_signature_under_the_negated_ring` below puts the
    same preimage to zkp's own verifier and needs the flagged extension for it.
    It pins the order against a `m || R || i || j` one, which is what this
    function computed, and what this test would have caught, before
    btclib-org/btclib#1070.
    """
    ...
    # the order btclib-org/btclib's preimage had before issue
    # btclib-org/btclib#1070, message first
...
def test_another_curve_signs_and_verifies(name: str) -> None:
    ...
    without ec, so every point was computed on secp256k1 -- ec.G being merely a
    *point* argument -- and then encoded against ec. The first bytes_from_point
    raised "y-coordinate not in 1..p-1" with a secp256k1-sized coordinate in the
    message, so no curve but secp256k1 could sign at all, and the corner case
    issue btclib-org/btclib#183 asked about was unreachable rather than
    untested.
    """
    ...
    # a BorromeanSig argument carries its own ec, as ssa.Sig does, so verify's
    # ec default is not what decides the curve here -- omitting ec=ec still
    # verifies. The octets it serializes to do not: parsed with ec defaulting to
    # secp256k1, every s is read ec13_11's few bytes short of secp256k1's
    # n_size, so this is where "the ec argument has to reach the arithmetic"
    # still bites. Refused rather than answered about: those octets are no
...
def test_a_zero_e_is_a_one_in_n_event_on_a_low_cardinality_curve() -> None:
    """The corner case e = 0 mod n, and no signature (btclib-org/btclib#183).
    ...
    # and both guards in verification, where nothing is random: the first e of a
    # ring, and one derived from a preceding r. Both e0 values are specific to
    # btclib-org/btclib#1070's preimage order -- e || m || ring || pos -- and
...
def test_the_point_at_infinity_is_the_other_corner_case() -> None:
    ...
    # this e0, like the ones above, is specific to btclib-org/btclib#1070's
    # preimage order and was found by searching
...
def test_a_ring_shape_that_disagrees_with_pubk_rings_is_refused() -> None:
    ...
    up". `verify` is asserted for each case too, and not only `assert_as_valid`:
    it is documented to answer `False` for an invalid signature, and only
    calling it exercises that promise for this input.
    """
    ec = secp256k1
    q1 = mult(1, ec.G, ec)
    q2 = mult(2, ec.G, ec)
...
def test_a_ring_with_no_keys_is_refused() -> None:
    ...
    `_assert_matches_pubk_rings` only compares the *counts* of two arguments, so
    ...
    reached -- and so does `sign`'s bound on `sign_key_idx[i]`, which a
...
def test_sign_key_idx_out_of_range_is_refused() -> None:
    ...
    `sign`'s shape check compares the *counts* of `pubk_rings`,
    `sign_key_idx`, `ks` and `sign_keys`, which all agree here -- one entry
    each. Unbounded, what `sign_key_idx[0]` itself points at would go wrong
    both ways: too large a value walks `s[i][j - 1]` and `pubk_rings[i][j - 1]`
    past the end of the ring's tuple in step 2, a bare `IndexError` (issue
...
def test_a_key_and_a_nonce_are_spelled_the_way_ecc_spells_a_scalar() -> None:
    ...
    `Sequence[int]` was this signer's alone, so a caller holding a key as octets
    or hex converted for it and for nothing else -- and the conversion they
    skipped did not raise this package's own error but Python's, all of them out
    of the one line that builds the real s-value: an `OverflowError` where the
...
# the real signer. "bit_length" and "value" are the two an unreduced real
# s fails on -- twice the bits and, following from that, easily the
...
def _assert_no_statistic_of_s_correlates_with_the_signer(
    ...
    correlation: against an unreduced real s the bit_length statistic
    scores close to 1.0, not chance plus a little.
    """
...
def test_no_statistic_of_s_correlates_with_the_signer() -> None:
    ...
    Unreduced, the real signer's s-value is about twice the bit length of
    the forged ones, so `max(s, key=bit_length)` names the signer with no
    cryptanalysis at all, which is what issue btclib-org/btclib#1053 found
    in btclib-org/btclib's borromean module. Reducing the real value and
    drawing the forged ones from the same range removes the signal; this
    asserts that removal statistically instead of taking it on faith.
    """
...
def test_no_statistic_of_s_correlates_with_the_signer_on_a_low_cardinality_curve() -> (
    ...
    """The same property where drawing from the scalars is what holds it.
...
def test_e0_hashes_the_ring_points_before_the_message() -> None:
    ...
    flagged extension for it (issue btclib-org/btclib#1940).
...
# `pragma: no cover` on the `@needs_zkp` below, the marker being the reason:
# `tests/conftest.py` turns it into a skip in an unflagged build, and excluding
# what a build cannot execute is what leaves the floor a measurement of the
# suite rather than of the build the machine has (issue btclib-org/btclib#1885).
# The marker's line and not the `def` under it, as `tests/ecc/pedersen_test.py`
...
def test_zkp_verifies_this_signature_under_the_negated_ring() -> None:
    ...
    What ``sign_`` writes is then refused under the ring and taken under that
    ring negated key by key. The negation is the whole of the distance between
    the two: `e0` is over the preimage zkp recomputes (issue
    btclib-org/btclib#1940), and the public key of a ring is the other one of
    the pair (issue btclib-org/btclib#1895).
...
def test_verify_tells_octets_that_are_no_signature_from_one_that_fails() -> None:
    """Issue btclib-org/btclib#2170's two sides, over the serialized signature.
