...
def test_challenge_hash_matches_zkps_preimage_order() -> None:
    ...
    # btclib-org/btclib#1070 had not been fixed
...
def test_sign_key_idx_out_of_range_is_refused() -> None:
    ...
    each. Nothing bounded what `sign_key_idx[0]` itself pointed at: too large a
    value walked `s[i][j - 1]` and `pubk_rings[i][j - 1]` past the end of the
    ring's tuple in step 2, a bare `IndexError` (issue btclib-org/btclib#1095);
    a negative one wrapped to a Python-legal index and signed a position other
    than the one it named, silently producing a signature that does not verify
    rather than raising anything at all. Both are now
    `EllipticCurvesValueError`, naming the ring, its size and the index, before
    either step runs.
    """
    ec = secp256k1
    q1 = mult(1, ec.G, ec)
