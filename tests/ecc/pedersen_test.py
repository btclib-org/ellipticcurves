...
`btclib_secp256k1.zkp` and compared; a commitment this package computed then
goes to `zkp.rangeproof`, which proves it, verifies the proof and rewinds it
back to the value and the blinding factor this package chose.

That gives this tree no rangeproof. The proof, its verification and its
rewind are libsecp256k1-zkp's throughout, and what this package contributes is
...
def test_second_generator_is_cached() -> None:
    """Issue btclib-org/btclib#287: (ec, hf) is the cache key, not ec alone.
...
# has (issue btclib-org/btclib#1885). The marker's line and not the `def` under
...
def test_zkp_rangeproof_over_this_package_s_commitment() -> None:
    ...
    and the blinding factor it hands back are the ones this package chose,
    recovered from a proof about a point this package computed.
...
def test_verify_tells_an_opening_that_is_no_number_from_one_that_fails() -> None:
    """Issue btclib-org/btclib#2170's two sides, over the opening.
    ...
    # the curve: all three answers about the value (issue btclib-org/btclib#814)
