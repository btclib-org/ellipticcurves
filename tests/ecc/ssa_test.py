...
def test_signature() -> None:
    ...
    # wrong with it is which point it is. It read as a type error until the
    # spelling dispatch stopped guessing (issue btclib-org/btclib#1188)
    ...
    # r outside the field and r inside it with no point are one refusal now,
    # phrased by Sig rather than by the lift it no longer takes (issue
    # btclib-org/btclib#622): "x-coordinate not in 0..p-1" was ec.y's, reached
...
def bip340_vectors() -> list[Any]:
    ...
    2023-04, of 0, 1, 17 and 100 bytes. They were the Python path's alone while
    a 32-byte gate stood in front of the bindings (issue btclib-org/btclib#169);
    it is gone, and all nineteen are signed and verified by libsecp256k1.
    """
    return [
...
def test_refusing_an_r_takes_no_square_root(
    ...
    `ec.y_even_var` for an x the bindings reject, that being where the message
    naming the value came from, so a bad r paid the whole Python square root,
    several times what verifying a good signature costs (issue
    btclib-org/btclib#622). Half of the field elements are no x-coordinate and
    cost nothing to produce, so that was the expensive answer for the cheap
    input.
...
def test_the_sighash_type_of_a_witness_signature_is_the_callers() -> None:
    ...
    which is what this package's own script engine does with `signature[:64]`
