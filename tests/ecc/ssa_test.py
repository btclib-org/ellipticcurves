...
def test_signature_on_an_equal_curve() -> None:
    ...
    # the two paths have one answer to agree on (issue btclib-org/btclib#142)
...
def test_signature() -> None:
    ...
    # phrased by Sig rather than by a lift it does not take (issue
    ...
    # does not sign (issue btclib-org/btclib#169)
    ...
    # size, so the length gates nothing
...
def test_parse_takes_64_bytes_and_no_other_number(
    ...
    Unrefused, sixty-three of them would parse into a signature of their own --
    an s read out of thirty-one bytes is below the order, so it is a valid
    scalar -- and sixty-five into the signature of the first sixty-four, the
    last byte read as part of nothing. Both are one object with two encodings,
    and the second is the shape a taproot witness signature arrives in.
    """
...
def test_refusing_an_r_takes_no_square_root(
    ...
    Refusing through a lift costs more than verifying: `_y_even_var` falls
    back to `ec.y_even_var` for an x the bindings reject, that being where the
    message naming the value comes from, so a bad r would pay the whole Python
    square root, several times what verifying a good signature costs (issue
    btclib-org/btclib#622). Half of the field elements are no x-coordinate and
    cost nothing to produce, so that would be the expensive answer for the
    cheap input.

    mod_sqrt_var is patched to raise rather than timed: what is asserted is
    a refusal that reaches no square root, on this machine and on a loaded
    one alike.

    Five r, because the refusal covers what a lift phrases as two distinct
    messages: `invalid x-coordinate` for a field element no point has, and
    `x-coordinate not in 0..p-1` for one outside the field. p - 1 is the last
    r that is inside it.
    """
...
def test_the_sighash_type_of_a_witness_signature_is_the_callers() -> None:
    ...
    The 65th octet is the sighash type, a fact about the transaction rather than
    part of the signature, and a `verify` reading the first sixty-four and
    dropping the rest would answer True with the byte still attached. Stripping
    it is the caller's, which is what btclib-org/btclib's script engine does
    with `signature[:64]` once `get_hashtype` has read it.
    """
...
def test_an_extended_key_is_no_longer_a_bip340_key() -> None:
    """Both text spellings of a public xpub, refused (btclib-org/btclib#1188).

    An extended key is BIP32's object, and turning one into a public key
    is not this package's call to make. `ssa` parses the spellings it
...
def test_a_buffer_is_octets_at_every_size_this_takes() -> None:
    """A bytearray and a memoryview, at each size (btclib-org/btclib#1188).
...
def test_octets_of_no_key_size_name_every_size_there_is() -> None:
    """Three sizes are accepted, so three are named (btclib-org/btclib#1188).
...
def test_a_message_of_any_size_reaches_the_bindings(
    ...
    Nothing in an answer says which implementation produced it, which is how the
    32-byte gate in front of the bindings went unnoticed until four of BIP340's
    own vectors fell through it (issue btclib-org/btclib#169). So the two calls
    are recorded here and the record is asserted: a gate put back would leave it
    empty for every size but 32, while every assertion about the signatures
    still passed.
    ...
    # are not parsed twice (issue btclib-org/btclib#887)
...
def test_the_delegated_arm_forwards_verify(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    The recorder above holds that `sign_custom` is called and forwards the
    keyword -- a dropped one raises `TypeError` there, its own local
    `sign_custom` declaring `verify` with no default -- but every call it makes
    is at the caller's own default, `verify=True`. Nothing held the *value* that
    crosses to be the caller's own rather than one written over on the way
    (issue btclib-org/btclib#986): BIP340 is the one scheme
    btclib-org/btclib#984 argues must keep the check on, which is exactly what a
    silent `verify=True` written at this crossing would still pass.
    """
    real_sign_custom = libsecp256k1_ssa.sign_custom
    calls: list[bool] = []
...
def test_verify_with_a_message_that_is_not_32_bytes_on_both_arithmetics(
    ...
    """Issue btclib-org/btclib#169's message, on both arithmetics.

    BIP340 takes a message of any size, and so does every path here:
    `sign_custom` and `verify` take one, and no 32-byte gate stands in front of
    them -- one would make `_assert_as_valid_` the only path able to verify four
    of BIP340's own vectors. So this exercises the bindings first and the Python
    arithmetic second, under `no_bindings`, because the two must not disagree
    about a signature and only one of them is reached by default. The Python one
    is reached in earnest by another curve or another hash function; its
    multiplication is still the bindings' on secp256k1, several times cheaper
    than the arithmetic under it.
    """
...
def test_batch_validation() -> None:
    ...
    # dropping a byte from a message is not a size error but a
    # different message, so the batch fails to verify rather than refusing
    # to look (issue btclib-org/btclib#169)
...
def test_one_message_is_not_a_batch_of_them() -> None:
    ...
    A caller passing the one message it has, rather than a batch holding it,
    would otherwise zip through its bytes and challenge each as its own message
    (issue btclib-org/btclib#1405). Only `msgs` is `Sequence[Octets]`; `Qs` and
    `sigs` are refused the way `_assert_batch_sequences`'s own `Sequence` check
    already refused a `None` (issue btclib-org/btclib#814), untouched here.
    """
    aux = b"\x00" * 32
    m = b"\x00" * 16
    q, Q = ssa.gen_keys(1)
...
def test_a_batch_of_one_takes_the_single_signature_shortcut(
    ...
    does not tell them apart, and the dispatch is what is asserted here
...
def test_batch_and_single_verification_agree_about_a_non_canonical_s(
    ...
    """One answer, whatever the batch size (btclib-org/btclib#688).
    ...
    shows is supported, and is what the batch equation would be asked
...
def test_batch_validation_on_the_python_path(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    Batch verification is this package's own: libsecp256k1 exposes no batch
...
def test_libsecp256k1() -> None:
    """Check this package's signature is byte-identical to the bindings' own."""
    msg = b"Satoshi Nakamoto"
...
def test_a_plain_sign_reaches_the_bindings(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    `sign_`'s guard -- `_libsecp256k1_serves(ec, hf) and commit_hash is None` --
    is what every caller with no commitment keeps by default, and both arms it
    chooses between reach the same signature from the same secret key: nothing
    about the result says which one produced it (issue btclib-org/btclib#975,
    asked in general of every guard a computed value could disable rather than
    of this one, which has none). `ssa.sign` for secp256k1 with sha256, any
    message size and no commitment is one call SECURITY.md names as crossing the
    boundary whole; the Python arm below composes the same signature from
    `bip340_nonce_` and a linear combination instead, its own two point
    multiplications still reaching libsecp256k1 while it serves. This records
    the call into `libsecp256k1_ssa.sign_custom` instead of the answer it
    returns, so a change that silently stopped taking it would fail here rather
...
def test_libsecp256k1_x_only_conversion() -> None:
    """The x-only key handed to the bindings is this package's to derive.
...
def test_recover_infinity_pub_key(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    issue btclib-org/btclib#286 turns on: a libsecp256k1 pubkey is never the
    identity, so the sum is recognized from the coordinates in curves.curve and
    handed back as the z == 0 this function tests -- u*H + v*Q with v*Q == -u*H
...
def test_recovery_multiplies_in_libsecp256k1(
    ...
    """x-only recovery on both point arithmetics (issue btclib-org/btclib#286).
...
def test_a_bad_hf_raises_rather_than_answering_about_the_signature() -> None:
    ...
    size but one (issue btclib-org/btclib#745).
    """
    msg = b"Satoshi Nakamoto"
    q, x_Q = ssa.gen_keys(0x1)
    sig = ssa.sign(msg, q)
...
def test_a_key_that_is_no_x_coordinate_is_refused_in_the_lift_s_words(
    ...
    The delegated path never lifts the key -- the verification's own parse is
    that square root (issue btclib-org/btclib#887) -- so the two messages are
    written in two places and would drift apart unnoticed but for this: what
    would notice is a caller matching on the text.
...
def test_signer_signs_what_sign_signs(
    ...
    with ssa.Signer(prv_key) as signer:
        ...
        # a *prepared* message of any size, which is the whole of what the
        # trailing underscore promises and what the bindings' 32-byte entry
        # point would take away again (issue btclib-org/btclib#169): signing
        # through that one would leave the two arms of this class disagreeing,
...
def test_verify_raises_on_a_structurally_invalid_signature_or_key() -> None:
    """A wrong length or spelling raises, not False (btclib-org/btclib#2170).

    BIP340's own reference raises on exactly these two: a signature or a
    public key of the wrong length. `verify` and `verify_` do not wrap the
    whole of `assert_as_valid`/`assert_as_valid_` in one `try`, which would
...
def test_batch_verify_raises_on_a_structurally_invalid_pub_key() -> None:
    """The batch spelling asks each key the same (btclib-org/btclib#2170).
