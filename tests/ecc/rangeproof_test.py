...
def test_verify_tells_octets_that_are_no_proof_from_one_that_fails() -> None:
    """Issue btclib-org/btclib#2170's two sides, over the proof's own octets.
...
def test_a_built_proof_is_asked_what_a_parse_asks_the_octets() -> None:
    """What `check_validity=False` makes reachable (btclib-org/btclib#2182).

    `RangeProof.parse` asks `assert_valid` of octets and `_proof_from` asks it
    of a `RangeProof` a caller built, which is how `ecc.borromean` and `ecc.ssa`
    ask their own object argument for it. So `assert_as_valid` and `rewind`
    refuse with this library's own exception, and `verify` answers False --
    where octets that are no proof raise out of `verify` instead (issue
    btclib-org/btclib#2170), an object having no octets to be wrong.
...
def test_extra_commit_is_read_as_octets_wherever_it_is_taken() -> None:
    ...
    `verify` catches the `ValueError` a spelling that is no octets raises:
    `extra_commit` is what the proof is written *about*, so it stays on issue
    btclib-org/btclib#814's side of the line, where the proof's own octets moved
    to the other one (issue btclib-org/btclib#2170). The three spellings that
    raise their reason let it out either way.
    """
    value = 100000
    commitment = commit(_BLIND, value, _GEN)
