...
def test_der_sequence_length_is_der_for_every_curve() -> None:
    ...
    secp256k1 and every other bitcoin curve stay under the 128-octet threshold
    where the two agree; `bpp512r1`, `nistp521` and `secp521r1` do not (issue
    btclib-org/btclib#2130). A self-consistency check across the whole
    catalogue, not the oracle: see `test_der_matches_a_vendored_der_oracle` for
    that.
    """
...
def test_der_matches_a_vendored_der_oracle() -> None:
    ...
    Outside means written by nobody in this change: `_der_length` above and
    `dsa._der_length` share an author and an afternoon, so a decoder or a copy
    of the encoder written beside the fix could share a misreading of X.690 and
    prove nothing (issue btclib-org/btclib#2130). This asserts against
    `tests/ecc/_data/der_length_vectors.json` alone -- `cryptography`'s
    `encode_dss_signature`, run once outside this tree and vendored rather than
    imported live, since `cryptography` is not a dependency any group the suite
    installs resolves. See `tests/_data/README.md` for the producing command.
    """
...
def test_der_sequence_length_refuses_the_long_form() -> None:
    ...
    accepting both would be new malleability (issue btclib-org/btclib#2130).
    """
...
def test_der_agrees_with_libsecp256k1_except_where_it_is_stricter() -> None:
    ...
    This package refuses the encoding instead, in BIP66's words, and this is
