...
Two operations have no such oracle, and that absence is recorded here rather
than left for a reader to notice on their own, per btclib-org/btclib#993's rule
that a missing third-party vector is said so in the inventory instead of being
counted twice: `nonce_gen_` derives a nonce libsecp256k1's own `musig_nonce_gen`
derives differently, so nothing below crosses the derivation, only the
arithmetic relating a pubnonce to a secnonce that
`tests.ecc.musig2_test.test_sign_verify_vectors_consistency` and
`test_tweak_vectors_consistency` already check with this package's own `mult`;
...
`musig_nonce_gen_counter` is a different construction, for a non-repeating
counter rather than for collapsing the two rounds -- so `det_sign_vectors.json`
is and stays its only authority.
...
def test_key_sort_vectors() -> None:
    ...
    # this package sorts into a new list where the reference sorts in place
...
def test_sig_agg_valid_vectors(case: dict[str, Any]) -> None:
    ...
    # the aggregate is an ordinary BIP340 signature, and this package's own
...
# The bindings oracle, issue btclib-org/btclib#1048: every test below
# cross-validates this package's answer against `libsecp256k1_musig`, reusing
# the vectors and the helpers already loaded above rather than a second copy of
# either.
...
# this package's own tests, past the vectors: the whole interactive protocol,
# once through, over a message that is not 32 bytes -- which is the precondition
# issue btclib-org/btclib#190 names, BIP327 signing a message of any size and
# the bindings taking none but a 32-byte one (issue btclib-org/btclib#169)
...
# issue btclib-org/btclib#1049: partial_sig_verify_'s delegated arm, exercised
# past what the BIP327 vectors reach on their own. _MSG32 rather than _MSG above
...
def test_partial_sig_verify_reuses_the_bindings_session() -> None:
    ...
    `SessionContext` on every call, which is why none of the vector-driven tests
    above can exercise this: they all go through that spelling. This calls
    `partial_sig_verify_` directly, twice, against one `session_ctx` -- the
    first call builds the (KeyAggCache, Session) pair and caches it there, the
    second reuses it, which is the whole point of caching it on the session
    rather than rebuilding it once per signer (issue btclib-org/btclib#1049's
...
def test_musig2_matches_zkp_with_no_adaptor() -> None:
    ...
    two sessions describing different sessions to begin with: this package
...
def test_musig2_adaptor_matches_zkp() -> None:
    """A pre-signature made here completes and extracts the same as zkp's.
    ...
    pass to `adapt`/`extract_adaptor`; this package derives the identical bit
    ...
    for _ in range(64):
        ...
        # completion: this package's own adapt agrees with zkp's, byte for byte
        sig = musig2.adapt(pre_sig, t, session_ctx)
...
def test_key_agg_coeff_cache_is_invisible_to_equality() -> None:
    ...
    Issue btclib-org/btclib#1069's `L`, `second` and `pub_keys_set` are folded
    into the same `_values` `test_session_values_cache_is_invisible_to_equality`
    above already pins, rather than a second cached field on `SessionContext` --
    so this is the same invariant, checked with two signers so that `second` is
    a real key rather than the one-signer placeholder, and populated through
    `partial_sig_verify_`, `_session_key_agg_coeff`'s other caller.
    """
    pk_1 = musig2.individual_pub_key(_SK_1)
    pk_2 = musig2.individual_pub_key(_SK_2)
    pub_keys = musig2.key_sort([pk_1, pk_2])
