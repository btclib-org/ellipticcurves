...
evidence that matters for a scheme with no specification: this package
...
def test_decrypt_electrum_ciphertext(plaintext: bytes, armor: str) -> None:
    """Verify this package reads what Electrum wrote.
...
def test_rebuild_electrum_ciphertext(plaintext: bytes, armor: str) -> None:
    """Verify this package writes what Electrum wrote, byte for byte.
    ...
    against this package itself could never do.
    """
    prv_key = _electrum_key_from_password(_ELECTRUM_PASSWORD)
...
def test_envelope_fields_are_octets_like_every_other_field() -> None:
    ...
    Declared `bytes` and assigned as they came, they would make two answers
    wrong: a value with no `len` would leave `assert_valid` as a TypeError about
    a builtin, and a value *with* one would be reported as a size -- a
    two-character str is not a two-byte magic, it is not a magic at all. Every
    other octet field of the library coerces in its constructor, so a hex string
    is what a caller may hold here too.
    """
    key_m = bytes(range(32))
