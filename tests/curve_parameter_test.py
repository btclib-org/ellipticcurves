...
what that walk is built to do without. `hf` is gated by hand for that
reason, where its own check lives. This is the table for the other of
the two, and it is the wider.

Issue btclib-org/btclib#868 is where the count is, and `ec` had no check of its
own when this file was written: every function below answered a wrong one with
`AttributeError: 'NoneType' object has no attribute 'n'`, a field read off
whatever arrived, which is the third of the four shapes issue
btclib-org/btclib#856 named.
...
`parse` and `serialize` never declare `ec`, so the whole family is absent from
the table below -- not an omission, a rule: `serialization_boundary_test.py`'s
module docstring states it and why, and `test_the_family_takes_no_ec_or_hf` is
its gate (issue btclib-org/btclib#1084).
"""
...
def _curve_parameters() -> set[str]:
    ...
    The **annotation** and not the name `ec`: a walk keyed on the
    spelling would leave a curve parameter spelled otherwise out of the
    table.

    `Curve` and `CurveGroup` exactly, and not a union containing one: a
    function taking a union compares against a curve rather than
    computing in one.
