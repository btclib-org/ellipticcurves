...
group, and fails to collect in CI -- issue btclib-org/btclib#1538's
shape, and the reason `tests/copyright_test.py` reads a line of
`docs/source/conf.py` rather than executing it. `json` is the standard
library and adds no group. What is read without a parser is narrow
enough to defend: the lines a key indents under it, one at a time, and
for `profiles` the fold a `>-` scalar performs on them, which is a join
on one space.
