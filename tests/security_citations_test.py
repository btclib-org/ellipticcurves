...
SECURITY.md publishes what this package protects and what it does not,
...
That is worse than a file with no pointers (btclib-org/btclib#1690).
...
def _enclosing(source: Path, lineno: int) -> str:
    ...
    citation the way it reads -- `commit_nonce.commit_nonce_` rather than
...
def test_a_broken_citation_of_each_kind_is_named(
    ...
    what btclib-org/btclib#1690 measured in SECURITY.md itself: a function named
