...
The guard used to be a shape match: "N files." opening a line, and a bullet
under `## Summary` opening with a digit. Both had a gap a widened pattern would
only have moved -- the noun ("response bodies", "vectors", a spelled-out
"seven") and the section (a `###` heading or the Provenance prose are not `##
Summary` and were never checked) -- and the general form of "a count in prose"
is not regular, so a pattern loose enough to catch every wrong shape also
catches sentences that are not wrong (btclib-org/btclib#1633). What is
enumerated instead is what is *permitted*: every line below was found by running
the pattern above against this file as it stands and reading what it caught, not
by predicting the shape a violation would take.
...
def test_the_pattern_still_matches() -> None:
    ...
    strings btclib-org/btclib#1633 measured the old, shape-based patterns
    missing; the other two are what the module's own guard caught before the
    rewrite and must go on catching after it.
    """
    for text in (
