...
"libsecp256k1 answers a bool and this is this package's own wording for it"
...
matrix rather than one runner, and `test-passed` gates it without a
...
def test_a_credit_wrapped_across_two_comment_lines_is_one_run() -> None:
    ...
    spread over two of them: a comment that spells a libsecp256k1
    function on its first line and names the library lines further down
    is a credit that no one line of it makes.
    """
    absent = "secp256k1_no_such_function"
    wrapped = "# which is libsecp256k1's own\n# " + absent + ", in prose\n"
...
def test_this_package_s_own_dispatch_name_is_not_read_as_the_library() -> None:
