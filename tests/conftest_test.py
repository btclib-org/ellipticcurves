...
"""Tests for the coverage gate and the skip hook.

`coverage_fail_under` is what a passing suite cannot exercise on its
own: the run that reaches it with a subset selected is, by
...
@pytest.mark.parametrize(
    ...
    [
        (["tests/curves/curve_test.py::test_one"], None, None, False),
        (None, ["tests/curves"], None, False),
...
def test_cov_is_not_the_last_token_of_addopts() -> None:
    ...
    and report 0.00% against a `fail_under` of 100.
