...
class _CountingGroup(CurveGroup):
    ...
    The number of additions is what the scalar used to decide (issue
    btclib-org/btclib#254), and counting them is how the test below says it no
    longer does: a wall-clock timing per multiplication is too noisy to resolve
    a spread of one addition in seventy, which is why the test counts the
...
def test_mult_on_a_characteristic_7_curve() -> None:
    ...
    (issue btclib-org/btclib#171). Before it was fixed, _mult_jac_var answered
    wrong for 6 of the 13 scalars here, the ladder and _mult_recursive_jac_var
    for 12 of 13, and _double_mult_var for 100 of the 169 coefficient pairs; the
    two fixed windows happened not to, every scalar of a curve this small
    fitting in one base-16 digit -- which is why the reference here is
    _mult_aff_var, the one multiplication that forms no Jacobian point at all.
    """
