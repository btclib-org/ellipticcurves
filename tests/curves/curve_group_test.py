...
class _CountingGroup(CurveGroup):
    ...
    The number of additions is what the scalar decides for a window that is
    not regular (issue btclib-org/btclib#254), and counting them is how the
    test below says it does not for the regular one: a wall-clock timing per
...
def test_regular_window_addition_count_is_the_same_for_every_scalar() -> None:
    """One count of each, not a range (btclib-org/btclib#254).
    ...
    additions the scalar decides for the fixed window.
    """
    ec = _CountingGroup()
...
def test_mult_on_a_characteristic_7_curve() -> None:
    ...
    INFJ is (7, 0, 0), so p == 7 is where that arbitrary x reduces to zero and
    add_jac's doubling test can mistake the identity for the other operand
    (issue btclib-org/btclib#171). Before btclib-org/btclib fixed it, its
    _mult_jac_var answered wrong for 6 of the 13 scalars here, the ladder and
    _mult_recursive_jac_var for 12 of 13, and _double_mult_var for 100 of the
    169 coefficient pairs; the two fixed windows happened not to, every scalar
    of a curve this small fitting in one base-16 digit -- which is why the
    reference here is _mult_aff_var, the one multiplication that forms no
    Jacobian point at all.
    """
    ec = Curve(7, 0, 3, (1, 2), 13, 1, False)
    # named pairs rather than the functions alone: three of them take a
    # window width, which carries no default, and a partial has no
...
def test_multi_mult_dispatch() -> None:
    ...
    for size in (BOS_COSTER_THRESHOLD - 1, BOS_COSTER_THRESHOLD):
        ...
        # both by name, the dispatch not being able to reach
...
def test_multi_mult_distant_magnitudes() -> None:
    """btclib-org/btclib#175, asked of the implementation that can still hang.
...
def test_the_interleaved_loop_makes_the_operations_its_wnafs_name(
    ...
    What `_additions_by_position` reorganized is where a digit is looked up, not
    what is done with it (issue btclib-org/btclib#906), and a comparison of
    results would be blind to a reordering that answers the same point -- group
    addition commutes. So the operations are asserted: the sequence of doublings
    and additions the call makes, *with the point each addition takes*, against
