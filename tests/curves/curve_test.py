...
def test_secp256k1_py_vectors(vector: dict[str, str]) -> None:
    ...
    assert bytes_from_point(mult(q), compressed=False) == pubkey_uncp
    assert bytes_from_point(mult(q), compressed=True) == pubkey_comp
...
# json data behind it, writes a prime (issue btclib-org/btclib#166)
...
def test_curves_with_n_above_p() -> None:
    """Hasse admits n > p, and catalogued curves do (btclib-org/btclib#183).

    The neighbour of the n == p check that issue btclib-org/btclib#166 found
...
def test_add_jac_infinity_is_not_a_doubling() -> None:
    ...
    add_jac's doubling test compares affine coordinates, and Z == 0 leaves X and
    Y free to be anything at all: INFJ is (7, 0, 0) and `_jac_from_aff(INF)` is
    (5, 0, 0), those x-coordinates picked for being invalid rather than for
    being zero. On a curve of characteristic 7 or 5 they reduce to zero, the
    test reads "same x, same y" and doubles -- issue btclib-org/btclib#171,
    where P + INFJ answered 2*P. The all-zero triple does it on every curve,
    secp256k1 included.
    """
    for ec in (
...
def test_add_aff_takes_infinity_before_doubling() -> None:
    ...
    btclib-org/btclib#171): INF is (5, 0) and its x-coordinate is arbitrary, so
    a doubling test reading it would answer "same x, different y, hence INF" for
    INF + P whenever P has x == 5. ec23_19 is generated by (5, 4), which is
    exactly that point, and every curve whose generator shares an x-coordinate
    with INF is.
    """
...
def test_multi_mult() -> None:
    """Verify multi_mult_var against double_mult_var.
    ...
    # issue btclib-org/btclib#175: a scalar pair of distant magnitude is what
    # the subtractive Bos-Coster step could not finish, and a mixed sign is
...
def no_bindings_anywhere(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    `no_bindings` above answers for `curve.py`'s own names and the one class
    beside them, which is what every arm gated on the curve and the hash
    function alone reaches through. An arm gated on availability alone can hold
    a binding of its own a module further out -- `ecc.dsa`'s own
    `_libsecp256k1_recover_sec_` binds `recovery` -- and `from ... import x as
...
def test_libsecp256k1_arbitrary_point() -> None:
    ...
    Every multiplication of secp256k1 goes through libsecp256k1, the
...
def test_x_coordinate_lift(bindings: bool, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    `_y_even_var` ask (issue btclib-org/btclib#284) -- and `ec.y_even_var` is
    what they are held against, over the 400 smallest field elements, of which
    208 are not x-coordinates at all. It is those that have to be checked: an
    implementation accepting one would take a public key consensus refuses, and
    the two do not even fail alike -- the bindings raise a bare ValueError where
    this package names the offending value.
