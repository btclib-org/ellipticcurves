...
def test_a_key_of_no_octets_is_no_key(size: int) -> None:
    """A size below one is refused, where a negative one would answer b"".

    The loop bound is computed from the size, so unrefused a negative one would
    make the loop empty and the final negative slice return nothing at all: a
    key derivation function answering with an empty key is the one answer it
    must never invent. Zero is refused with it -- SEC 1 3.6.1 states keydatalen
