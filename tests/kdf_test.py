...
rule is that this package never *chooses* SHA-1: no default, no derivation of
its own reaches for it. A KDF here chooses no hash function at all --
`hf` is the caller's, and `ansi_x9_63_kdf` is driven over
`[sha1, sha256, sha512]` below. What SHA-1 appears in is third-party
vectors: RFC 5869's appendix A publishes seven, four of them SHA-1, and
the CAVP's ANSI-X9.63 component vectors run from SHA-1 to SHA-512.
Running the SHA-256 half of a published set and dropping the rest would
be editing the authority rather than citing it, which is what
`tests/_data/README.md`'s provenance discipline exists to prevent. The
cost is test data in this file: no dependency, and no production path.
"""
...
def test_a_key_of_no_octets_is_no_key(size: int) -> None:
    ...
    The loop bound is computed from the size, so a negative one made the loop
    empty and the final negative slice returned nothing at all: a key derivation
    function answering with an empty key is the one answer it must never invent.
    Zero is refused with it -- SEC 1 3.6.1 states keydatalen as a positive
    integer, and a caller asking for no octets of keying material has a bug
    rather than an empty key (issue btclib-org/btclib#321).
    """
