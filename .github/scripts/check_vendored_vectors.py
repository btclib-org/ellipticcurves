...
btclib carries a copy under this same name, over a ledger of the same
shape, and btclib-secp256k1 another, over its own tests/README.md, whose
entries are each pinned to a commit under a heading owning one fenced
block. What the copies share is owed to the others in the same
campaign: the parsing, the `gh` calls, a field's spelling, the arguments
this takes. What answers to one ledger's own shape is not, and the
collapsing of identical skip lines below is that -- a heading owning
several blocks being a shape btclib-secp256k1's README does not carry.
...
`ref` line is what lets such a pin be checked instead of excused by a
`behind` line this script never revisits.
