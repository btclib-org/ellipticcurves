...
- the bindings also let a caller own the buffer a secret is written
    ...
    (`src/ellipticcurves/ecc/dsa.py:1442`) crosses the same boundary the
...
- **the delegated multiplication of a point that is not the generator is
    ...
    (`src/ellipticcurves/curves/curve.py:798`). `dh.diffie_hellman` at
    ...
    (`src/ellipticcurves/curves/curve.py:1244`):
...
- a sign-to-contract commitment is the signer's to open, and opening it
    ...
    protocol, which `dsa`'s `anti_exfil_*` functions carry: there, the
