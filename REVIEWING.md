...
- Does the diff make a module of this package **import anything of the
  organization's other packages**? `CONTRIBUTING.md`'s *The public
  surface* states it, and `tests/imports_test.py` is the gate.
- Does a change move a computation with a secret — a private key, a
  nonce — **between the libsecp256k1 bindings and the Python
  arithmetic**? `SECURITY.md` says what the Python arithmetic does not
  promise.
